...
interface OwQuestionTranslation {
  id: number;
  ...
  created_at: string;
  updated_at: string;
  one_word_questions?: { code: string; slug?: string } | null;
...
type SortField = 'id' | 'created_at' | 'one_word_question_id' | 'language_id';
...
export default function OwQuestionTranslationsPage() {
  const [items, setItems] = useState<OwQuestionTranslation[]>([]);
  const [owQuestions, setOwQuestions] = useState<OwQuestion[]>([]);
  ...
  const [viewItem, setViewItem] = useState<OwQuestionTranslation | null>(null);

  const toolbarRef = useRef<DataToolbarHandle>(null);
  const router = useRouter();

  // --- Auto-fetch translation when question or language changes ---
  const watchedQuestionId = watch('one_word_question_id');
  ...
  useEffect(() => {
    ...
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?one_word_question_id=${watchedQuestionId}&language_id=${watchedLangId}&limit=1`;
      const res = await api.listOwQuestionTranslations(qs);
      if (cancelled) return;

      if (res.success && res.data && res.data.length > 0) {
        const item = res.data[0] as OwQuestionTranslation;
        setEditing(item);
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterQuestion) qs.set('one_word_question_id', filterQuestion);
      if (filterLanguage) qs.set('language_id', filterLanguage);
      if (filterStatus) qs.set('is_active', filterStatus);
    }
    qs.set('sort', sortField);
    qs.set('order', sortOrder);
    const res = await api.listOwQuestionTranslations('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  const defaultFormValues = {
    one_word_question_id: '', language_id: '',
    question_text: '', correct_answer: '', hint: '', explanation: '',
    is_active: true,
    image_1: '', image_2: '',
  };

  function clearFormFields() {
    const qid = getValues('one_word_question_id');
    const lid = getValues('language_id');
    Object.entries(defaultFormValues).forEach(([k, v]) => setValue(k, v));
    setValue('one_word_question_id', qid);
    setValue('language_id', lid);
    resetImageState();
  }

  function populateForm(item: OwQuestionTranslation) {
    setValue('question_text', item.question_text || '');
    setValue('correct_answer', item.correct_answer || '');
  ...
  function openCreate() {
    ...
    reset({ ...defaultFormValues, one_word_question_id: owQuestions[0]?.id || '', language_id: languages[0]?.id || '' });
    setDialogOpen(true);
  }

  function openEdit(item: OwQuestionTranslation) {
    skipAutoFetchRef.current = true;
    setEditing(item); setFormMode('existing'); resetImageState(); setDialogKey(k => k + 1);
    reset({
      one_word_question_id: item.one_word_question_id, language_id: item.language_id,
  ...
  async function onSubmit(data: any) {
    ...
    // Scalar text fields
    const scalarFields = [
      'one_word_question_id', 'language_id', 'question_text', 'correct_answer',
      'hint', 'explanation', 'image_1', 'image_2',
    ];
    ...
    const res = editing
      ? await api.updateOwQuestionTranslation(editing.id, fd, true)
      : await api.createOwQuestionTranslation(fd, true);
    if (res.success) {
  ...
  async function handleBulkSoftDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.deleteOwQuestionTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkRestore() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.restoreOwQuestionTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.permanentDeleteOwQuestionTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="One Word Question Translations" description="Manage translations for one-word answer questions"
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : Languages} title={showTrash ? 'Trash is empty' : 'No translations yet'} description={showTrash ? 'Deleted translations will appear here' : 'Add your first one-word question translation'}
            ...
            <Table>
              <THead>
                <TR className="hover:bg-transparent">
                  ...
                  <TH><button onClick={() => handleSort('one_word_question_id')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">Question <SortIcon field="one_word_question_id" /></button></TH>
              ...
              <TBody>
                {items.map(item => (
                  <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                    ...
                    <TD className="py-2.5">
                      {item.one_word_questions?.code ? (
                        <div>
                          <Badge variant="info">{item.one_word_questions.code}</Badge>
                          {item.one_word_questions.slug && <span className="block text-xs text-slate-400 mt-0.5">{item.one_word_questions.slug}</span>}
                        </div>
                    ...
                    <TD className="py-2.5">
                      {item.correct_answer ? (
                        <Badge variant="success">{item.correct_answer.length > 30 ? item.correct_answer.substring(0, 30) + '...' : item.correct_answer}</Badge>
      ...
      {/* --- View Dialog --- */}
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} title="View One Word Question Translation" size="lg">
        {viewItem && (
          <div className="p-6 space-y-4">
            {/* Header info */}
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{owQuestions.find(q => q.id === viewItem.one_word_question_id)?.code || `Question #${viewItem.one_word_question_id}`}</Badge>
            ...
            <div className="space-y-3">
              ...
              <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-3">
                <span className="block text-xs font-semibold text-emerald-700 mb-1">Correct Answer</span>
                <p className="text-base font-semibold text-emerald-900">{viewItem.correct_answer || <span className="text-slate-300 italic font-normal">Not set</span>}</p>
              </div>
      ...
      {/* --- Edit / Create Dialog --- */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit One Word Question Translation' : 'Add One Word Question Translation'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getOwQuestionTranslation(editing.id);
          ...
          <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4">
            <label className="block text-sm font-semibold text-emerald-800 mb-1">Correct Answer <span className="text-red-500">*</span></label>
            <input type="text" className={cn(selectClass, 'w-full border-emerald-300 focus:ring-emerald-500')} placeholder="The correct one-word answer..." {...register('correct_answer', { required: true })} />
            <p className="text-xs text-emerald-600 mt-1">This is the expected correct answer for this question translation.</p>
          </div>

          {/* Explanation */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Explanation</label>
            <textarea className={cn(selectClass, 'w-full min-h-[80px]')} placeholder="Explanation for the correct answer..." {...register('explanation')} />
          </div>

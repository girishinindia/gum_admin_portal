...
import {
  ...
  Eye, ArrowUpDown, ArrowUp, ArrowDown, Search, AlertTriangle, BarChart3, ChevronRight
} from 'lucide-react';
...
export default function CreateDescriptivePage() {
  ...
  const [answerType, setAnswerType] = useState('short_answer');
  ...
  // ── List state ──
  const [questions, setQuestions] = useState<DescListItem[]>([]);
  ...
  const [filterAnswerType, setFilterAnswerType] = useState('');
  ...
  const loadSummary = useCallback(async () => {
    try {
      const res = await api.getTableSummary('descriptive_questions');
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (showTrash) {
        ...
        if (filterAnswerType) qs.set('answer_type', filterAnswerType);
        if (filterDifficulty) qs.set('difficulty_level', filterDifficulty);
        if (filterStatus) qs.set('is_active', filterStatus);
      }
      if (searchDebounce) qs.set('search', searchDebounce);

      const r = await api.listDescQuestions('?' + qs.toString());
      if (r.success) {
        setQuestions(r.data || []);
  ...
  }, [topicId, page, pageSize, sortField, sortOrder, showTrash, searchDebounce, filterAnswerType, filterDifficulty, filterStatus]);

  useEffect(() => { loadQuestions(); }, [loadQuestions]);

  // ── Reset page and selection when filters change ──
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [filterAnswerType, filterDifficulty, filterStatus, showTrash, searchDebounce, topicId, pageSize]);
  ...
  const startEdit = async (id: number) => {
    try {
      const r = await api.getDescQuestionFull(id);
      if (!r.success || !r.data) { toast.error('Failed to load question'); return; }
      const q = r.data;
      setEditingId(id);
      setAnswerType(q.answer_type);
      ...
      setMinWords(q.min_words ?? '');
      setMaxWords(q.max_words ?? '');
  ...
  const handleSaveTranslation = async () => {
    ...
    try {
      ...
      const listRes = await api.listDescQuestionTranslations(`?descriptive_question_id=${editingId}&language_id=${editLangId}&limit=1`);
      const existing = listRes.success && listRes.data && listRes.data.length > 0 ? listRes.data[0] : null;

      if (existing && existing.id) {
        // Update existing question translation
        await api.updateDescQuestionTranslation(existing.id, {
        ...
        await api.createDescQuestionTranslation({
          descriptive_question_id: editingId,
      ...
      // Reload the full question to refresh allTranslations
      const r = await api.getDescQuestionFull(editingId);
      if (r.success && r.data) {
        const q = r.data;
  ...
  const handleSave = async () => {
    ...
    if (!questionText.trim()) { toast.error('Question text is required'); return; }
    if (!answerType) { toast.error('Answer type is required'); return; }
    ...
    try {
      const payload = {
        ...
        is_active: isActive,
        min_words: minWords === '' ? null : minWords,
        max_words: maxWords === '' ? null : maxWords,
      };

      let r;
      if (mode === 'edit' && editingId) {
        r = await api.updateFullDescQuestion(editingId, payload);
      } else {
        r = await api.createFullDescQuestion(payload);
      }

      if (r.success) {
        toast.success(mode === 'edit' ? 'Question updated!' : 'Question created with English translation!');
        resetForm();
        loadQuestions();
        loadSummary();
      } else {
  ...
  const handleAiTranslate = async (questionId?: number) => {
    ...
    try {
      const r = await api.autoTranslateDesc({ question_ids: [qId] });
      if (r.success) {
        toast.success('AI translations generated!');
        // Reload coverage and translations
        if (editingId) {
          const full = await api.getDescQuestionFull(editingId);
          if (full.success) {
            ...
            loadLanguageIntoForm(editLangId, full.data.question_translations || []);
          }
        }
        loadQuestions();
      } else {
  ...
  const handleAiTranslateAll = async () => {
    ...
    try {
      ...
      const r = await api.autoTranslateDesc(payload);
      if (r.success) {
  ...
  const handleAiTranslateSelected = async () => {
    ...
    setAiTranslating(true);
    try {
      const r = await api.autoTranslateDesc({ question_ids: Array.from(selectedIds) });
      if (r.success) {
  ...
  const handleSoftDelete = async (id: number) => {
    ...
    try {
      const r = await api.deleteDescQuestion(id);
      if (r.success) {
  ...
  const handleRestore = async (id: number) => {
    setActionLoadingId(id);
    try {
      const r = await api.restoreDescQuestion(id);
      if (r.success) {
  ...
  const handlePermanentDelete = async (id: number) => {
    ...
    setActionLoadingId(id);
    try {
      const r = await api.permanentDeleteDescQuestion(id);
      if (r.success) {
  ...
  const handleBulkSoftDelete = async () => {
    ...
    for (let i = 0; i < ids.length; i++) {
      const res = await api.deleteDescQuestion(ids[i]);
      if (res.success) ok++;
  ...
  const handleBulkRestore = async () => {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.restoreDescQuestion(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  const handleBulkPermanentDelete = async () => {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.permanentDeleteDescQuestion(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  const handleDeleteTranslation = async (langId: number) => {
    ...
    try {
      ...
      if (qTrans?.id) {
        await api.permanentDeleteDescQuestionTranslation(qTrans.id);
      }
      ...
      if (editLangId === langId) {
        setEditLangId(7);
        // reload English data
        const engTrans = (r?.data?.question_translations || allTranslations).find((t: any) => t.language_id === 7);
        setQuestionText(engTrans?.question_text || '');
        setHintText(engTrans?.hint || engTrans?.hint_text || '');
        setExplanationText(engTrans?.explanation || engTrans?.explanation_text || '');
      }
  ...
  const handleDeleteAllTranslations = async () => {
    if (!editingId) return;
    const nonEngLangs = translationCoverage.filter(c => c.language_id !== 7 && c.has_question_translation);
  ...
  const translatedLangs = translationCoverage.filter(c => c.has_question_translation).length;
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Create, manage, and translate descriptive questions in one place"
        actions={
          mode === 'list' && !showTrash ? (
      ...
      {mode === 'list' && (
        <DataToolbar
          ...
          searchPlaceholder={showTrash ? 'Search trash...' : 'Search descriptive questions...'}
        >
          {!showTrash && (
            <>
              ...
              <select className="h-10 px-3 pr-8 text-sm rounded-lg border border-slate-200 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 appearance-none cursor-pointer" value={filterAnswerType} onChange={e => setFilterAnswerType(e.target.value)}>
                <option value="">All Types</option>
                {ANSWER_TYPE_OPTIONS.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
      ...
      {mode === 'list' ? (
        ...
        <div className="space-y-4">
          {listLoading ? (
            ...
            <EmptyState
              ...
              title={showTrash ? 'Trash is empty' : 'No descriptive questions yet'}
              description={showTrash ? 'No deleted questions' : (searchDebounce || filterAnswerType || filterDifficulty || filterStatus ? 'No questions match your filters' : topicId ? 'Add your first descriptive question' : 'Select a subject, chapter, and topic to create questions — or browse all questions below')}
              action={!showTrash && !searchDebounce && !filterAnswerType && !filterDifficulty && !filterStatus && topicId ? <Button onClick={startCreate}><Plus className="w-4 h-4" /> Add question</Button> : undefined}
            />
          ) : (
              ...
              <Table>
                <THead>
                  <TR className="hover:bg-transparent">
                    ...
                    <TH className="w-28">Word Limits</TH>
                    ...
                    <TR key={q.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(q.id) && 'bg-brand-50/40')}>
                      ...
                      <TD className="py-2.5">
                        <Badge className={cn('text-xs', {
                          'bg-blue-50 text-blue-700': q.answer_type === 'short_answer',
                          'bg-violet-50 text-violet-700': q.answer_type === 'long_answer',
                        })}>
                          {ANSWER_TYPE_OPTIONS.find(t => t.value === q.answer_type)?.label || q.answer_type}
                        </Badge>
                      </TD>
                      <TD className="py-2.5">
                        <Badge className={cn('text-xs', {
                      ...
                      <TD className="py-2.5 text-center text-sm text-gray-600">
                        {q.min_words != null || q.max_words != null
        ...
        <div className="bg-white border rounded-lg">
            ...
            <div className="px-6 pt-3 border-b bg-gray-50">
              <div className="flex gap-1 overflow-x-auto">
                {availableLanguages.map(lang => {
                  ...
                  const hasTranslation = coverage?.has_question_translation;
                  return (
                    <button
                      key={lang.id}
          ...
          <div className="p-6 space-y-6">
            {/* Row 1: Answer Type + Difficulty + Points/Order/Active/Required */}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Model Answer ({currentLangName})
              </label>
              <textarea
                className="w-full border rounded-md px-3 py-2 text-sm min-h-[80px]"
                value={answerText}
                onChange={e => setAnswerText(e.target.value)}
                placeholder={`The model answer in ${currentLangName}...`}
              />
            </div>
              ...
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                <div className="flex flex-wrap gap-2">
                  {translationCoverage.map(c => {
                    const complete = c.has_question_translation;
                    return (

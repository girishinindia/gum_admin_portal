...
type SortField = 'id' | 'created_at' | 'desc_question_id' | 'language_id';
...
export default function DescQuestionTranslationsPage() {
  ...
  // --- Read desc_question_id from URL search params to pre-filter ---
  useEffect(() => {
    const urlDescQuestionId = searchParams.get('desc_question_id');
  ...
  const watchedQuestionId = watch('desc_question_id');
  ...
  useEffect(() => {
    ...
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?desc_question_id=${watchedQuestionId}&language_id=${watchedLangId}&limit=1`;
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterQuestion) qs.set('desc_question_id', filterQuestion);
  ...
  function openCreate() {
    ...
    reset({ ...defaultFormValues, desc_question_id: descQuestions[0]?.id || '', language_id: languages[0]?.id || '' });
    setDialogOpen(true);
  }
  ...
  function openEdit(item: DescQuestionTranslation) {
    ...
    reset({
      desc_question_id: item.desc_question_id, language_id: item.language_id,
  ...
  async function onSubmit(data: any) {
    ...
    // Scalar text fields
    const scalarFields = [
      'desc_question_id', 'language_id', 'question_text', 'answer_text',
            ...
            <Table>
              <THead>
                <TR className="hover:bg-transparent">
                  ...
                  <TH><button onClick={() => handleSort('desc_question_id')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">Question <SortIcon field="desc_question_id" /></button></TH>
        ...
        {viewItem && (
          <div className="p-6 space-y-4">
            ...
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{descQuestions.find(q => q.id === viewItem.desc_question_id)?.code || `Question #${viewItem.desc_question_id}`}</Badge>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select className={cn(selectClass, 'w-full')} {...register('desc_question_id', { required: true })}>
                {descQuestions.map(q => <option key={q.id} value={q.id}>{q.code}{q.slug ? ` (${q.slug})` : ''}</option>)}
              </select>
            </div>
            <div>

...
import { Plus, Link2, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3, RotateCcw, AlertTriangle, Loader2, Check, X } from 'lucide-react';
...
export default function MatchingQuestionsPage() {
  ...
  useEffect(() => {
    api.getTableSummary('matching_questions').then(res => {
  ...
  async function load() {
    ...
    const res = await api.listMatchingQuestions('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  async function onSubmit(data: any) {
    ...
    const res = editing
      ? await api.updateMatchingQuestion(editing.id, payload)
      : await api.createMatchingQuestion(payload);
    if (res.success) {
      toast.success(editing ? 'Matching question updated' : 'Matching question created');
  ...
  async function onSoftDelete(c: any) {
    ...
    const res = await api.deleteMatchingQuestion(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Matching question moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(c: any) {
    setActionLoadingId(c.id);
    const res = await api.restoreMatchingQuestion(c.id);
    setActionLoadingId(null);
  ...
  async function onPermanentDelete(c: any) {
    ...
    setActionLoadingId(c.id);
    const res = await api.permanentDeleteMatchingQuestion(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Matching question permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(c: any) {
    const res = await api.updateMatchingQuestion(c.id, { is_active: !c.is_active });
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Manage matching pair questions for assessments"
        actions={
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={showTrash ? 'No deleted questions' : (searchDebounce || filterStatus || filterTopicId || filterDifficulty ? 'No questions match your filters' : 'Add your first matching question')}
            ...
            <TBody>
              {items.map(c => (
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    <span className="text-sm font-medium text-slate-700">{c.pair_count ?? '--'}</span>
                  </TD>
                  {!showTrash && (
                    <TD className="py-2.5">
      ...
      {/* ── View Matching Question Dialog ── */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Matching Question Details" size="lg">
        {viewing && (
          <div className="p-6">
            {/* Header */}
            ...
            <div className="grid grid-cols-3 gap-x-8 gap-y-4">
              ...
              <DetailRow label="Pair Count" value={viewing.pair_count != null ? String(viewing.pair_count) : undefined} />
      ...
      {/* ── Create / Edit Dialog ── */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Matching Question' : 'Add Matching Question'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getMatchingQuestion(editing.id);
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input label="Code" placeholder="match-math-001" {...register('code', { required: true })} />
              <Input label="Slug" placeholder="match-math-001" {...register('slug', { required: true, onChange: () => setSlugManual(true) })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>

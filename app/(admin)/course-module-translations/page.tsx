...
interface CourseModuleTranslation {
  id: number;
  ...
  created_at: string;
  updated_at: string;
  course_modules?: { slug?: string; name?: string; course_id?: number; courses?: { code?: string; slug?: string; name?: string } | null } | null;
...
const TABS = ['Basic', 'Content', 'SEO', 'OG', 'Twitter', 'Images'] as const;
...
export default function CourseModuleTranslationsPage() {
  const [items, setItems] = useState<CourseModuleTranslation[]>([]);
  const [courseModules, setCourseModules] = useState<CourseModule[]>([]);
  const [languages, setLanguages] = useState<Language[]>([]);
  const [coverage, setCoverage] = useState<CoverageItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<CourseModuleTranslation | null>(null);
  const [dialogKey, setDialogKey] = useState(0);
  const [filterCourseModule, setFilterCourseModule] = useState('');
  ...
  const [viewItem, setViewItem] = useState<CourseModuleTranslation | null>(null);
  const [viewTab, setViewTab] = useState<typeof TABS[number]>('Basic');

  const toolbarRef = useRef<DataToolbarHandle>(null);
  const router = useRouter();

  // ─── Auto-fetch translation when course_module or language changes ───
  const watchedCourseModuleId = watch('course_module_id');
  ...
  useEffect(() => {
    ...
    if (!watchedCourseModuleId || !watchedLangId) return;

    let cancelled = false;
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?course_module_id=${watchedCourseModuleId}&language_id=${watchedLangId}&limit=1`;
      const res = await api.listCourseModuleTranslations(qs);
      if (cancelled) return;

      if (res.success && res.data && res.data.length > 0) {
        const item = res.data[0] as CourseModuleTranslation;
        setEditing(item);
  ...
  }, [watchedCourseModuleId, watchedLangId]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    api.listCourseModules('?limit=500&sort=display_order&ascending=true').then(res => { if (res.success) setCourseModules((res.data || []).filter((m: CourseModule) => m.is_active)); });
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterCourseModule, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]);
  useEffect(() => { load(); setSelectedIds(new Set()); }, [searchDebounce, page, filterCourseModule, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]); // eslint-disable-line react-hooks/exhaustive-deps

  async function refreshSummary() {
    const res = await api.getTableSummary('course_module_translations');
    if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
  }

  async function loadCoverage() {
    const res = await api.getCourseModuleTranslationCoverage();
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterCourseModule) qs.set('course_module_id', filterCourseModule);
      if (filterLanguage) qs.set('language_id', filterLanguage);
      if (filterStatus) qs.set('is_active', filterStatus);
    }
    qs.set('sort', sortField);
    qs.set('order', sortOrder);
    const res = await api.listCourseModuleTranslations('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  const defaultFormValues = {
    ...
    // Image URL field — file handled separately
    image: '',
  };

  function clearFormFields() {
    const cmid = getValues('course_module_id');
    const lid = getValues('language_id');
    Object.entries(defaultFormValues).forEach(([k, v]) => setValue(k, v));
    setValue('course_module_id', cmid);
    setValue('language_id', lid);
    resetImageState();
  }

  function populateForm(item: CourseModuleTranslation) {
  ...
  function openCreate() {
    ...
    reset({ ...defaultFormValues, course_module_id: courseModules[0]?.id || '', language_id: languages[0]?.id || '' });
    setDialogOpen(true);
  }

  function openEdit(item: CourseModuleTranslation) {
    skipAutoFetchRef.current = true;
    setEditing(item); setFormMode('existing'); resetImageState(); setDialogKey(k => k + 1); setActiveTab('Basic');
    reset({
      course_module_id: item.course_module_id, language_id: item.language_id,
      name: item.name || '', short_intro: item.short_intro || '', description: item.description || '',
  ...
  async function onSubmit(data: any) {
    ...
    // Scalar text fields
    const scalarFields = [
      'course_module_id', 'language_id', 'name', 'short_intro', 'description',
      'sort_order',
    ...
    const res = editing
      ? await api.updateCourseModuleTranslation(editing.id, fd, true)
      : await api.createCourseModuleTranslation(fd, true);
    if (res.success) {
  ...
  async function onSoftDelete(item: CourseModuleTranslation) {
    if (!confirm(`Move "${item.name}" to trash? You can restore it later.`)) return;
    setActionLoadingId(item.id);
    const res = await api.deleteCourseModuleTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Translation moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(item: CourseModuleTranslation) {
    setActionLoadingId(item.id);
    const res = await api.restoreCourseModuleTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`"${item.name}" restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(item: CourseModuleTranslation) {
    if (!confirm(`PERMANENTLY delete "${item.name}"? This cannot be undone.`)) return;
    setActionLoadingId(item.id);
    const res = await api.permanentDeleteCourseModuleTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Translation permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(item: CourseModuleTranslation) {
    const fd = new FormData();
    fd.append('is_active', String(!item.is_active));
    const res = await api.updateCourseModuleTranslation(item.id, fd, true);
    if (res.success) { toast.success(`${!item.is_active ? 'Activated' : 'Deactivated'}`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  function openView(item: CourseModuleTranslation) {
    setViewItem(item);
    setViewTab('Basic');
    setViewOpen(true);
  }
  ...
  async function handleBulkSoftDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.deleteCourseModuleTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkRestore() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.restoreCourseModuleTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.permanentDeleteCourseModuleTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  function getCoverage(courseModuleId: number): CoverageItem | undefined {
    return coverage.find(c => c.course_module_id === courseModuleId);
  }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <DataToolbar ref={toolbarRef} search={search} onSearchChange={setSearch} searchPlaceholder={showTrash ? 'Search trash...' : 'Search translations...'}>
        {!showTrash && (
          <>
            <select className={selectClass} value={filterCourseModule} onChange={e => setFilterCourseModule(e.target.value)}>
              <option value="">All course modules</option>
              {courseModules.map(m => <option key={m.id} value={m.id}>{m.name || m.slug}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : BookOpen} title={showTrash ? 'Trash is empty' : 'No translations yet'} description={showTrash ? 'Deleted translations will appear here' : 'Add your first course module translation'}
        ...
        <>
          <div className={cn('mt-4 bg-white rounded-xl border overflow-hidden shadow-sm', showTrash ? 'border-amber-200' : 'border-slate-200')}>
            ...
            <Table>
              ...
              <TBody>
                {items.map(item => {
                  const cov = getCoverage(item.course_module_id);
                  return (
                    <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                      ...
                      <TD className="py-2.5">{item.course_modules?.name ? <Badge variant="info">{item.course_modules.name}</Badge> : <span className="text-slate-300">--</span>}</TD>
      ...
      {/* ─── View Dialog ─── */}
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} title="View Course Module Translation" size="lg">
        {viewItem && (
          <div className="p-6 space-y-4">
            {/* Header info */}
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{viewItem.course_modules?.name || `Module #${viewItem.course_module_id}`}</Badge>
            ...
            {viewTab === 'Images' && (
              <div className="space-y-3">
                <div>
                  ...
                  {viewItem.image ? (
                    <img src={viewItem.image} alt="image" className="max-h-40 rounded-lg border border-slate-200" />
                  ) : (
                    <p className="text-sm text-slate-300 italic">Not set</p>
                  )}
                </div>
      ...
      {/* ─── Edit / Create Dialog ─── */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Course Module Translation' : 'Add Course Module Translation'} size="lg">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getCourseModuleTranslation(editing.id);
          ...
          {activeTab === 'Basic' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Course Module</label>
                  <select className={cn(selectClass, 'w-full')} {...register('course_module_id', { required: true })}>
                    {courseModules.map(m => <option key={m.id} value={m.id}>{m.name || m.slug}</option>)}
                  </select>
                </div>
                <div>
              ...
              <Input label="Name" placeholder="Module name in target language" {...register('name', { required: true })} />
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Short Intro</label>
                <textarea className={cn(selectClass, 'w-full min-h-[80px]')} placeholder="Brief module introduction..." {...register('short_intro')} />
              </div>
          ...
          {activeTab === 'Content' && (
            <div className="space-y-4">
              <div>
                ...
                <textarea className={cn(selectClass, 'w-full min-h-[200px]')} placeholder="Detailed module description..." {...register('description')} />
              </div>
            </div>
          )}

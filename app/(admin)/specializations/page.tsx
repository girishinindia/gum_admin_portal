...
import { Plus, Compass, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3 } from 'lucide-react';
...
export default function SpecializationsPage() {
  ...
  useEffect(() => { setPage(1); }, [searchDebounce, filterCat, filterStatus, pageSize]);
  useEffect(() => { load(); }, [searchDebounce, page, pageSize, filterCat, filterStatus, sortField, sortOrder]);
  ...
  return (
    <div className="animate-fade-in">
      ...
      {loading ? (
        ...
        <EmptyState icon={Compass} title="No specializations yet"
          description={searchDebounce || filterCat || filterStatus ? 'No specializations match your filters' : 'Add your first specialization'}
          action={!searchDebounce && !filterCat && !filterStatus ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add specialization</Button> : undefined} />
      ) : (
          ...
          <Table>
            <THead>
              <TR className="hover:bg-transparent">
                ...
                <TH><button onClick={() => handleSort('category')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">Category <SortIcon field="category" /></button></TH>
                <TH>Description</TH>

...
import { Plus, GraduationCap, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3 } from 'lucide-react';
...
export default function EducationLevelsPage() {
  ...
  async function onDelete(l: EducationLevel) {
    ...
    if (res.success) { toast.success('Education level deleted'); load(); refreshSummary(); }
    ...
    <div className="animate-fade-in">
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={searchDebounce || filterCategory || filterStatus ? 'No education levels match your filters' : 'Add your first education level'}
          action={!searchDebounce && !filterCategory && !filterStatus ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add level</Button> : undefined}
        />
      ) : (

...
import { Plus, BookOpen, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3, RotateCcw, AlertTriangle, Loader2, X, Languages } from 'lucide-react';
...
export default function OwSynonymsPage() {
  ...
  useEffect(() => {
    setValue('one_word_question_id', '');
    if (formTopicId) {
      api.listOwQuestions(`?limit=999&sort=display_order&order=asc&topic_id=${formTopicId}`).then(res => {
  ...
  useEffect(() => {
    api.getTableSummary('one_word_synonyms').then(res => {
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterQuestionId) qs.set('one_word_question_id', filterQuestionId);
    }
    const res = await api.listOwSynonyms('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  async function refreshSummary() {
    const res = await api.getTableSummary('one_word_synonyms');
    if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
  }
  ...
  function getQuestionLabel(questionId: number) {
    const q = owQuestions.find(q => q.id === questionId);
  ...
  function openCreate() {
    ...
    reset({
      one_word_question_id: '', display_order: '', is_active: true,
    });
    setDialogOpen(true);
  }
  ...
  async function openEdit(c: any) {
    ...
    if (c.one_word_question_id) {
      const qRes = await api.getOwQuestion(c.one_word_question_id);
        ...
        if (topicRes.success && topicRes.data) {
          ...
          if (topic.chapter_id) {
            ...
            if (chapterRes.success && chapterRes.data) {
              ...
              const questionsRes = await api.listOwQuestions(`?limit=999&sort=display_order&order=asc&topic_id=${qRes.data.topic_id}`);
    ...
    reset({
      one_word_question_id: c.one_word_question_id ?? '',
  ...
  async function onSubmit(data: any) {
    ...
    Object.keys(data).forEach(k => {
      ...
      const numericFields = ['one_word_question_id', 'display_order'];
      if (numericFields.includes(k) && v !== '') { payload[k] = Number(v); return; }
      payload[k] = v;
    });

    const res = editing
      ? await api.updateOwSynonym(editing.id, payload)
      : await api.createOwSynonym(payload);
    if (res.success) {
      toast.success(editing ? 'Synonym updated' : 'Synonym created');
      setDialogOpen(false); load(); refreshSummary();
    } else toast.error(res.error || 'Failed');
  }

  async function onSoftDelete(c: any) {
    if (!confirm(`Move synonym #${c.id} to trash? You can restore it later.`)) return;
    setActionLoadingId(c.id);
    const res = await api.deleteOwSynonym(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Synonym moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(c: any) {
    setActionLoadingId(c.id);
    const res = await api.restoreOwSynonym(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`Synonym #${c.id} restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(c: any) {
    if (!confirm(`PERMANENTLY delete synonym #${c.id}? This cannot be undone.`)) return;
    setActionLoadingId(c.id);
    const res = await api.permanentDeleteOwSynonym(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Synonym permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(c: any) {
    const res = await api.updateOwSynonym(c.id, { is_active: !c.is_active });
  ...
  async function handleBulkSoftDelete() {
    ...
    for (let i = 0; i < ids.length; i++) {
      const res = await api.deleteOwSynonym(ids[i]);
      if (res.success) ok++;
  ...
  async function handleBulkRestore() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.restoreOwSynonym(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  async function handleBulkPermanentDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.permanentDeleteOwSynonym(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="One Word Synonyms"
        description="Manage alternative accepted answers"
        actions={
          <div className="flex items-center gap-2">
            {!showTrash && <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add synonym</Button>}
          </div>
        }
      />
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-4 mb-5">
          {[
            { label: 'Total Synonyms', value: summary.total, icon: BarChart3, color: 'bg-blue-50 text-blue-600' },
      ...
      <DataToolbar
        ...
        searchPlaceholder={showTrash ? 'Search trash...' : 'Search synonyms...'}
      >
        {!showTrash && (
          <>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={showTrash ? 'Trash is empty' : 'No synonyms yet'}
          description={showTrash ? 'No deleted synonyms' : (searchDebounce || filterStatus || filterQuestionId || filterSubjectId || filterChapterId || filterTopicId ? 'No synonyms match your filters' : 'Add your first synonym')}
          action={!showTrash && !searchDebounce && !filterStatus && !filterQuestionId && !filterSubjectId && !filterChapterId && !filterTopicId ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add synonym</Button> : undefined}
        />
      ) : (
                ...
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    <span className={cn('text-sm font-medium', showTrash ? 'text-slate-500 line-through' : 'text-slate-700')}>
                      {getQuestionLabel(c.one_word_question_id)}
                    </span>
                  </TD>
                  <TD className="py-2.5">
                    <span className={cn('text-sm', showTrash ? 'text-slate-500 line-through' : 'text-slate-900')}>
                      {c.synonym_text || <span className="text-slate-300">--</span>}
                    </span>
                  </TD>
                  <TD className="py-2.5">
                    {c.translation_count != null ? (
                      <button
                        onClick={() => router.push(`/ow-synonym-translations?one_word_synonym_id=${c.id}`)}
      ...
      {/* -- View Synonym Dialog -- */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Synonym Details" size="lg">
        {viewing && (
          <div className="p-6">
            {/* Header */}
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Synonym #{viewing.id}</h3>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <DetailRow label="Question" value={getQuestionLabel(viewing.one_word_question_id)} />
              <DetailRow label="Synonym Text (English)" value={viewing.synonym_text} />
              ...
              <DetailRow label="Question ID" value={viewing.one_word_question_id != null ? String(viewing.one_word_question_id) : undefined} />
            </div>
      ...
      {/* -- Create / Edit Dialog -- */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Synonym' : 'Add Synonym'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {editing.is_active ? 'This synonym is currently active' : 'This synonym is currently inactive'}
                </p>
              </div>
              <button
                type="button"
                onClick={async () => {
                  ...
                  const refreshed = await api.getOwSynonym(editing.id);
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">One Word Question</label>

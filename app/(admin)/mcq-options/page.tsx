...
import { Plus, ListChecks, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3, RotateCcw, AlertTriangle, Loader2, Check, X } from 'lucide-react';
import { cn, fromNow } from '@/lib/utils';
import { useKeyboardShortcuts } from '@/hooks/useKeyboardShortcuts';
import { usePageSize } from '@/hooks/usePageSize';

type SortField = 'id' | 'display_order' | 'is_correct' | 'is_active';

export default function McqOptionsPage() {
  ...
  const [filterIsCorrect, setFilterIsCorrect] = useState<string>('');
  ...
  // Form cascade: topic → questions
  useEffect(() => {
    setValue('mcq_question_id', '');
    if (formTopicId) {
      api.listMcqQuestions(`?limit=999&sort=display_order&order=asc&topic_id=${formTopicId}`).then(res => {
  ...
  useEffect(() => {
    api.getTableSummary('mcq_options').then(res => {
      if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
    });
  }, []);

  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterStatus, filterIsCorrect, filterSubjectId, filterChapterId, filterTopicId, filterQuestionId, pageSize, showTrash]);
  useEffect(() => { load(); setSelectedIds(new Set()); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterStatus, filterIsCorrect, filterSubjectId, filterChapterId, filterTopicId, filterQuestionId, showTrash]);
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterIsCorrect) qs.set('is_correct', filterIsCorrect);
      if (filterQuestionId) qs.set('mcq_question_id', filterQuestionId);
    }
    const res = await api.listMcqOptions('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  async function refreshSummary() {
    const res = await api.getTableSummary('mcq_options');
    if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
  }
  ...
  function getQuestionLabel(questionId: number) {
    const q = mcqQuestions.find(q => q.id === questionId);
  ...
  function openCreate() {
    ...
    reset({
      mcq_question_id: '', is_correct: false, display_order: '', is_active: true,
    });
    setDialogOpen(true);
  }
  ...
  async function openEdit(c: any) {
    ...
    if (c.mcq_question_id) {
      const qRes = await api.getMcqQuestion(c.mcq_question_id);
        ...
        if (topicRes.success && topicRes.data) {
          ...
          if (topic.chapter_id) {
            ...
            if (chapterRes.success && chapterRes.data) {
              ...
              const questionsRes = await api.listMcqQuestions(`?limit=999&sort=display_order&order=asc&topic_id=${qRes.data.topic_id}`);
    ...
    reset({
      mcq_question_id: c.mcq_question_id ?? '',
  ...
  async function onSubmit(data: any) {
    ...
    Object.keys(data).forEach(k => {
      ...
      const numericFields = ['mcq_question_id', 'display_order'];
      if (numericFields.includes(k) && v !== '') { payload[k] = Number(v); return; }
      payload[k] = v;
    });

    const res = editing
      ? await api.updateMcqOption(editing.id, payload)
      : await api.createMcqOption(payload);
    if (res.success) {
      toast.success(editing ? 'MCQ option updated' : 'MCQ option created');
      setDialogOpen(false); load(); refreshSummary();
    } else toast.error(res.error || 'Failed');
  }

  async function onSoftDelete(c: any) {
    if (!confirm(`Move option #${c.id} to trash? You can restore it later.`)) return;
    setActionLoadingId(c.id);
    const res = await api.deleteMcqOption(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('MCQ option moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(c: any) {
    setActionLoadingId(c.id);
    const res = await api.restoreMcqOption(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`Option #${c.id} restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(c: any) {
    if (!confirm(`PERMANENTLY delete option #${c.id}? This cannot be undone.`)) return;
    setActionLoadingId(c.id);
    const res = await api.permanentDeleteMcqOption(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('MCQ option permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(c: any) {
    const res = await api.updateMcqOption(c.id, { is_active: !c.is_active });
  ...
  async function handleBulkSoftDelete() {
    ...
    for (let i = 0; i < ids.length; i++) {
      const res = await api.deleteMcqOption(ids[i]);
      if (res.success) ok++;
  ...
  async function handleBulkRestore() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.restoreMcqOption(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  async function handleBulkPermanentDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.permanentDeleteMcqOption(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="MCQ Options"
        description="Manage answer options for MCQ questions"
        actions={
          <div className="flex items-center gap-2">
            {!showTrash && <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add option</Button>}
          </div>
        }
      />
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-4 mb-5">
          {[
            { label: 'Total Options', value: summary.total, icon: BarChart3, color: 'bg-blue-50 text-blue-600' },
      ...
      <DataToolbar
        ...
        searchPlaceholder={showTrash ? 'Search trash...' : 'Search MCQ options...'}
      >
        {!showTrash && (
          <>
            ...
            <select className={selectClass} value={filterIsCorrect} onChange={e => setFilterIsCorrect(e.target.value)}>
              <option value="">All Correctness</option>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={showTrash ? Trash2 : ListChecks}
          title={showTrash ? 'Trash is empty' : 'No MCQ options yet'}
          description={showTrash ? 'No deleted MCQ options' : (searchDebounce || filterStatus || filterIsCorrect || filterQuestionId || filterSubjectId || filterChapterId || filterTopicId ? 'No MCQ options match your filters' : 'Add your first MCQ option')}
          action={!showTrash && !searchDebounce && !filterStatus && !filterIsCorrect && !filterQuestionId && !filterSubjectId && !filterChapterId && !filterTopicId ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add option</Button> : undefined}
        />
      ) : (
          ...
          <Table>
            <THead>
              <TR className="hover:bg-transparent">
                ...
                <TH>Option Text</TH>
                <TH>
                  <button onClick={() => handleSort('is_correct')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">
                    Correct? <SortIcon field="is_correct" />
                  </button>
                </TH>
                <TH>Translations</TH>
                <TH>
                ...
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    <span className={cn('text-sm font-medium', showTrash ? 'text-slate-500 line-through' : 'text-slate-700')}>
                      {getQuestionLabel(c.mcq_question_id)}
                    </span>
                  </TD>
                  <TD className="py-2.5">
                    <span className={cn('text-sm', showTrash ? 'text-slate-500 line-through' : 'text-slate-900')}>
                      {c.option_text || <span className="text-slate-300">--</span>}
                    </span>
                  </TD>
                  <TD className="py-2.5">
                    {c.is_correct ? (
                      ...
                      <span className="inline-flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full bg-slate-50 text-slate-500">
                  ...
                  <TD className="py-2.5">
                    {c.translation_count != null ? (
                      <span className="text-xs font-semibold text-slate-600">{c.translation_count}</span>
                    ) : (
      ...
      {/* ── View MCQ Option Dialog ── */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="MCQ Option Details" size="lg">
        {viewing && (
          <div className="p-6">
            {/* Header */}
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                <h3 className="text-lg font-semibold text-slate-900">Option #{viewing.id}</h3>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              ...
              <DetailRow label="Question" value={getQuestionLabel(viewing.mcq_question_id)} />
              <DetailRow label="Option Text (English)" value={viewing.option_text} />
              <DetailRow label="Translation Count" value={viewing.translation_count != null ? String(viewing.translation_count) : undefined} />
              <DetailRow label="Is Correct" value={viewing.is_correct ? 'Yes' : 'No'} />
              ...
              <DetailRow label="Question ID" value={viewing.mcq_question_id != null ? String(viewing.mcq_question_id) : undefined} />
            </div>
      ...
      {/* ── Create / Edit Dialog ── */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit MCQ Option' : 'Add MCQ Option'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {editing.is_active ? 'This option is currently active' : 'This option is currently inactive'}
                </p>
              </div>
              <button
                type="button"
                onClick={async () => {
                  ...
                  const refreshed = await api.getMcqOption(editing.id);
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">MCQ Question</label>
                <select className={cn(selectClass, 'w-full')} {...register('mcq_question_id', { required: true })} disabled={!formTopicId}>
            ...
            <div className="space-y-2.5">
              <label className="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" className="w-4 h-4 rounded border-slate-300 text-brand-600 focus:ring-brand-500" {...register('is_correct')} />

...
export default function SubjectTranslationsPage() {
  ...
  async function handleAIGenerate() {
    const subjectId = getValues('subject_id');
    const langId = getValues('language_id');
    if (!subjectId || !langId) { toast.error('Please select a subject and language first'); return; }
    ...
    setAiLoading(true);
    try {
      const res = await api.generateSubjectTranslation({
  ...
  async function handleBulkGenerateContent() {
    ...
    try {
      ...
      items.filter(i => selectedIds.has(i.id)).forEach(i => {
        if (i.subject_id) parentIds.add(i.subject_id);
      });
  ...
  return (
    <div className="animate-fade-in">
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : BookOpen} title={showTrash ? 'Trash is empty' : 'No translations yet'} description={showTrash ? 'Deleted translations will appear here' : 'Add your first subject translation'}
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getSubjectTranslation(editing.id);
            ...
            {aiPanelOpen && (
              <div className="px-4 py-3 bg-white space-y-3">
                <p className="text-xs text-slate-500">
                  {selectedLangCode === 'en'
                    ? <>AI will generate <strong>English</strong> content (name, short intro, long intro) for the selected subject.</>
                ...
                <div>
                  ...
                  <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} disabled={aiLoading}
                    ...
                    placeholder={selectedLangCode === 'en'
                      ? 'e.g. Generate engaging educational content for this subject.'
                ...
                <AiProgressOverlay
                  active={aiLoading}
                  phases={['Analyzing subject content...', 'Generating translation with AI...', 'Processing response...', 'Preparing fields...']}

...
export default function CreateMatchingPage() {
  ...
  // ── List state ──
  const [questions, setQuestions] = useState<MatchingListItem[]>([]);
  ...
  const loadSummary = useCallback(async () => {
    try {
      const res = await api.getTableSummary('matching_questions');
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.listMatchingQuestions('?' + qs.toString());
      if (r.success) {
        setQuestions(r.data || []);
  ...
  const loadLanguageIntoForm = (langId: number, translations: any[], pairsData: any[]) => {
    ...
    // Build pairs with the selected language's text
    const builtPairs: PairItem[] = pairsData.map((pair: any) => {
      const pairTrans = (pair.translations || []).find((t: any) => t.language_id === langId);
      return {
        id: pair.id,
    ...
    setPairs(builtPairs.length >= 2 ? builtPairs : [{ left_text: '', right_text: '' }, { left_text: '', right_text: '' }]);
  };
  ...
  const startEdit = async (id: number) => {
    try {
      const r = await api.getMatchingQuestionFull(id);
      ...
      setPartialScoring(q.partial_scoring ?? false);

      // Store all translations and pairs for language switching
      const translations = q.question_translations || [];
      const pairsData = q.pairs || [];
      setAllTranslations(translations);
      setAllOptions(pairsData);
      ...
      // Build pairs from enriched data (English)
      const builtPairs: PairItem[] = pairsData.map((pair: any) => {
        const engPairTrans = (pair.translations || []).find((t: any) => t.language_id === 7);
        return {
          id: pair.id,
          left_text: engPairTrans?.left_text || '',
          right_text: engPairTrans?.right_text || '',
        };
      });
      setPairs(builtPairs.length >= 2 ? builtPairs : [{ left_text: '', right_text: '' }, { left_text: '', right_text: '' }]);
  ...
  // ── Update pair left text ──
  const updatePairLeft = (idx: number, text: string) => {
    setPairs(prev => prev.map((p, i) => i === idx ? { ...p, left_text: text } : p));
  };

  // ── Update pair right text ──
  const updatePairRight = (idx: number, text: string) => {
    setPairs(prev => prev.map((p, i) => i === idx ? { ...p, right_text: text } : p));
  };
  ...
  const handleSaveTranslation = async () => {
    ...
    if (pairs.some(p => !p.left_text.trim() || !p.right_text.trim())) { toast.error('All pairs must have both left and right text'); return; }
    ...
    try {
      ...
      if (existingQTrans && existingQTrans.id) {
        ...
        await api.updateMatchingQuestionTranslation(existingQTrans.id, {
        ...
        await api.createMatchingQuestionTranslation({
          matching_question_id: editingId,
      ...
      // Save pair translations
      for (let i = 0; i < pairs.length; i++) {
        const pair = pairs[i];
        const pairId = allOptions[i]?.id;
        if (!pairId) continue;

        // Find existing pair translation by listing
        const pairTransRes = await api.listMatchingPairTranslations(`?matching_pair_id=${pairId}&language_id=${editLangId}&limit=1`);
        const existingPairTrans = pairTransRes.success && pairTransRes.data && pairTransRes.data.length > 0
          ? pairTransRes.data[0]
          : null;

        if (existingPairTrans && existingPairTrans.id) {
          // Update existing pair translation
          await api.updateMatchingPairTranslation(existingPairTrans.id, {
            left_text: pair.left_text.trim(),
            right_text: pair.right_text.trim(),
          });
        } else {
          // Create new pair translation
          await api.createMatchingPairTranslation({
      ...
      const r = await api.getMatchingQuestionFull(editingId);
      if (r.success && r.data) {
        const q = r.data;
        setAllTranslations(q.question_translations || []);
        setAllOptions(q.pairs || []);
        setTranslationCoverage(q.translation_coverage || []);
      }
  ...
  const handleSave = async () => {
    ...
    if (!questionText.trim()) { toast.error('Question text is required'); return; }
    const validPairs = pairs.filter(p => p.left_text.trim() && p.right_text.trim());
    if (validPairs.length < 2) { toast.error('At least 2 complete pairs are required (both left and right text)'); return; }
    ...
    try {
      ...
      if (mode === 'edit' && editingId) {
        r = await api.updateFullMatchingQuestion(editingId, payload);
      } else {
        r = await api.createFullMatchingQuestion(payload);
      }

      if (r.success) {
        toast.success(mode === 'edit' ? 'Question updated!' : 'Question created with pairs & English translations!');
        resetForm();
        loadQuestions();
        loadSummary();
      } else {
  ...
  const handleAiTranslate = async (questionId?: number) => {
    ...
    try {
      const r = await api.autoTranslateMatching({ question_ids: [qId] });
      if (r.success) {
        toast.success('AI translations generated!');
        // Reload coverage and translations
        if (editingId) {
          const full = await api.getMatchingQuestionFull(editingId);
          if (full.success) {
            setTranslationCoverage(full.data.translation_coverage || []);
            setAllTranslations(full.data.question_translations || []);
            setAllOptions(full.data.pairs || []);
            // Reload current language into form
            loadLanguageIntoForm(editLangId, full.data.question_translations || [], full.data.pairs || []);
          }
        }
        loadQuestions();
      } else {
  ...
  const handleAiTranslateAll = async () => {
    ...
    try {
      ...
      const r = await api.autoTranslateMatching(payload);
      if (r.success) {
  ...
  const handleAiTranslateSelected = async () => {
    ...
    setAiTranslating(true);
    try {
      const r = await api.autoTranslateMatching({ question_ids: Array.from(selectedIds) });
      if (r.success) {
  ...
  const handleSoftDelete = async (id: number) => {
    ...
    try {
      const r = await api.deleteMatchingQuestion(id);
      if (r.success) {
  ...
  const handleRestore = async (id: number) => {
    setActionLoadingId(id);
    try {
      const r = await api.restoreMatchingQuestion(id);
      if (r.success) {
  ...
  const handlePermanentDelete = async (id: number) => {
    ...
    setActionLoadingId(id);
    try {
      const r = await api.permanentDeleteMatchingQuestion(id);
      if (r.success) {
  ...
  const handleBulkSoftDelete = async () => {
    ...
    for (let i = 0; i < ids.length; i++) {
      const res = await api.deleteMatchingQuestion(ids[i]);
      if (res.success) ok++;
  ...
  const handleBulkRestore = async () => {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.restoreMatchingQuestion(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  const handleBulkPermanentDelete = async () => {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.permanentDeleteMatchingQuestion(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  // ── Delete a single language's translation ──
  const handleDeleteTranslation = async (langId: number) => {
    ...
    try {
      ...
      if (qTrans?.id) {
        await api.permanentDeleteMatchingQuestionTranslation(qTrans.id);
      }
      for (const opt of allOptions) {
        const optTrans = (opt.translations || []).find((t: any) => t.language_id === langId);
        if (optTrans?.id) {
          await api.permanentDeleteMatchingPairTranslation(optTrans.id);
        }
      }
      ...
      if (r.success && r.data) {
        ...
        setAllOptions(r.data.pairs || []);
        setTranslationCoverage(r.data.translation_coverage || []);
      }
      if (editLangId === langId) {
        setEditLangId(7);
        loadLanguageIntoForm(7, r?.data?.question_translations || allTranslations, r?.data?.pairs || allOptions);
      }
    } catch (e: any) {
  ...
  const handleDeleteAllTranslations = async () => {
    if (!editingId) return;
    const nonEngLangs = translationCoverage.filter(c => c.language_id !== 7 && (c.has_question_translation || c.has_pair_translations));
  ...
  const translatedLangs = translationCoverage.filter(c => c.has_question_translation && c.has_pair_translations).length;
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Create, manage, and translate matching questions with pairs in one place"
        actions={
          mode === 'list' && !showTrash ? (
      ...
      {mode === 'list' && (
        <DataToolbar
          ...
          searchPlaceholder={showTrash ? 'Search trash...' : 'Search matching questions...'}
        >
          {!showTrash && (
            <>
      ...
      {mode === 'list' ? (
        ...
        <div className="space-y-4">
          {listLoading ? (
            ...
            <EmptyState
              ...
              title={showTrash ? 'Trash is empty' : 'No matching questions yet'}
              description={showTrash ? 'No deleted questions' : (searchDebounce || filterDifficulty || filterStatus ? 'No questions match your filters' : topicId ? 'Add your first matching question' : 'Select a subject, chapter, and topic to create questions — or browse all questions below')}
              action={!showTrash && !searchDebounce && !filterDifficulty && !filterStatus && topicId ? <Button onClick={startCreate}><Plus className="w-4 h-4" /> Add question</Button> : undefined}
            />
          ) : (
                ...
                <TBody>
                  {questions.map(q => (
                    <TR key={q.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(q.id) && 'bg-brand-50/40')}>
                      ...
                      <TD className="py-2.5 text-center">{q.pair_count}</TD>
                      <TD className="py-2.5">
                        {!showTrash ? (
        ...
        <div className="bg-white border rounded-lg">
            ...
            <div className="px-6 pt-3 border-b bg-gray-50">
              <div className="flex gap-1 overflow-x-auto">
                {availableLanguages.map(lang => {
                  ...
                  const hasTranslation = coverage?.has_question_translation && coverage?.has_pair_translations;
                  return (
                    <button
                      key={lang.id}
          ...
          <div className="p-6 space-y-6">
            {/* Row 1: Difficulty + Points + Order + Active/Required + Partial Scoring */}
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="flex items-end gap-3 pb-1">
                ...
                <label className={cn("flex items-center gap-1.5 text-sm", isNonEnglish ? "text-gray-400 cursor-not-allowed" : "cursor-pointer")}>
                  <input type="checkbox" checked={partialScoring} onChange={e => setPartialScoring(e.target.checked)} className="rounded" disabled={isNonEnglish} />
                  Partial Scoring
                </label>
              </div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-gray-700">
                  Matching Pairs ({currentLangName}) *
                </label>
                {!isNonEnglish && (
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <div className="flex-1">
                    <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Left Side</span>
                  </div>
                  <div className="flex-1">
                    <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">Right Side</span>
                  </div>
                  <div className="w-9" />
                </div>
                {pairs.map((pair, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <Input
                      ...
                      onChange={e => updatePairLeft(idx, e.target.value)}
                      placeholder={`Left item ${idx + 1} in ${currentLangName}...`}
                    />
                    <Input
                      className="flex-1"
                      value={pair.right_text}
                      onChange={e => updatePairRight(idx, e.target.value)}
                      placeholder={`Right item ${idx + 1} in ${currentLangName}...`}
                    />
                    {pairs.length > 2 && !isNonEnglish && (
                      <button
              ...
              {isNonEnglish && (
                <p className="text-xs text-gray-400 mt-2">
                  Adding/removing pairs can only be done in English.
                </p>
              )}
            </div>
              ...
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                <div className="flex flex-wrap gap-2">
                  {translationCoverage.map(c => {
                    const complete = c.has_question_translation && c.has_pair_translations;
                    const partial = c.has_question_translation || c.has_pair_translations;
                    return (

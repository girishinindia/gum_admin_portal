...
interface CoverageItem {
  ...
  has_item_translations: boolean;
}

interface OrderingListItem {
  id: number;
  code: string;
  slug: string;
...
export default function CreateOrderingPage() {
  ...
  const [items, setItems] = useState<OrderItem[]>([
    { item_text: '', correct_position: 1 },
    { item_text: '', correct_position: 2 },
  ]);
  ...
  // ── List state ──
  const [questions, setQuestions] = useState<OrderingListItem[]>([]);
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.listOrderingQuestions('?' + qs.toString());
      if (r.success) {
        setQuestions(r.data || []);
  ...
  const loadLanguageIntoForm = (langId: number, translations: any[], itemsData: any[]) => {
    ...
    // Build items with the selected language's text
    const builtItems: OrderItem[] = itemsData.map((item: any) => {
      const itemTrans = (item.translations || []).find((t: any) => t.language_id === langId);
      return {
        id: item.id,
    ...
    setItems(builtItems.length >= 2 ? builtItems : [{ item_text: '', correct_position: 1 }, { item_text: '', correct_position: 2 }]);
  };
  ...
  const startEdit = async (id: number) => {
    try {
      const r = await api.getOrderingQuestionFull(id);
      ...
      // Build items from enriched data (English)
      const builtItems: OrderItem[] = itemsData.map((item: any) => {
        const engItemTrans = (item.translations || []).find((t: any) => t.language_id === 7);
        return {
          id: item.id,
          item_text: engItemTrans?.item_text || '',
          correct_position: item.correct_position,
        };
      });
      setItems(builtItems.length >= 2 ? builtItems : [
        { item_text: '', correct_position: 1 },
        { item_text: '', correct_position: 2 },
      ]);
  ...
  // ── Add item ──
  const addItem = () => {
    setItems(prev => [...prev, { item_text: '', correct_position: prev.length + 1 }]);
  };

  // ── Remove item ──
  const removeItem = (idx: number) => {
    if (items.length <= 2) { toast.error('Minimum 2 items required'); return; }
  ...
  // ── Update item correct_position ──
  const updateItemPosition = (idx: number, pos: number) => {
    setItems(prev => prev.map((it, i) => i === idx ? { ...it, correct_position: pos } : it));
  };
  ...
  const handleSaveTranslation = async () => {
    ...
    if (items.some(it => !it.item_text.trim())) { toast.error('All items must have text'); return; }
    ...
    try {
      ...
      if (existingQTrans && existingQTrans.id) {
        ...
        await api.updateOrderingQuestionTranslation(existingQTrans.id, {
        ...
        await api.createOrderingQuestionTranslation({
          ordering_question_id: editingId,
      ...
      for (let i = 0; i < items.length; i++) {
        const item = items[i];
        const itemId = allOptions[i]?.id;
        if (!itemId) continue;

        // Check for existing item translation
        const existingItemTransRes = await api.listOrderingItemTranslations(
          `?ordering_item_id=${itemId}&language_id=${editLangId}&limit=1`
        );
        const existingItemTrans = existingItemTransRes.success && existingItemTransRes.data?.length > 0
          ? existingItemTransRes.data[0]
          : null;

        if (existingItemTrans && existingItemTrans.id) {
          // Update existing item translation
          await api.updateOrderingItemTranslation(existingItemTrans.id, {
            item_text: item.item_text.trim(),
          });
        } else {
          // Create new item translation
          await api.createOrderingItemTranslation({
      ...
      const r = await api.getOrderingQuestionFull(editingId);
      if (r.success && r.data) {
        const q = r.data;
        setAllTranslations(q.question_translations || []);
        setAllOptions(q.items || []);
        setTranslationCoverage(q.translation_coverage || []);
      }
  ...
  const handleSave = async () => {
    ...
    if (!questionText.trim()) { toast.error('Question text is required'); return; }
    const validItems = items.filter(it => it.item_text.trim());
    if (validItems.length < 2) { toast.error('At least 2 items with text are required'); return; }
    ...
    try {
      ...
      if (mode === 'edit' && editingId) {
        r = await api.updateFullOrderingQuestion(editingId, payload);
      } else {
        r = await api.createFullOrderingQuestion(payload);
      }

      if (r.success) {
        toast.success(mode === 'edit' ? 'Question updated!' : 'Question created with items & English translations!');
        resetForm();
        loadQuestions();
        loadSummary();
      } else {
  ...
  const handleAiTranslate = async (questionId?: number) => {
    ...
    try {
      const r = await api.autoTranslateOrdering({ question_ids: [qId] });
      if (r.success) {
        toast.success('AI translations generated!');
        // Reload coverage and translations
        if (editingId) {
          const full = await api.getOrderingQuestionFull(editingId);
          if (full.success) {
            setTranslationCoverage(full.data.translation_coverage || []);
            setAllTranslations(full.data.question_translations || []);
            setAllOptions(full.data.items || []);
            // Reload current language into form
            loadLanguageIntoForm(editLangId, full.data.question_translations || [], full.data.items || []);
          }
        }
        loadQuestions();
      } else {
  ...
  const handleAiTranslateAll = async () => {
    ...
    try {
      ...
      const r = await api.autoTranslateOrdering(payload);
      if (r.success) {
  ...
  const handleAiTranslateSelected = async () => {
    ...
    setAiTranslating(true);
    try {
      const r = await api.autoTranslateOrdering({ question_ids: Array.from(selectedIds) });
      if (r.success) {
  ...
  const handleDeleteTranslation = async (langId: number) => {
    ...
    try {
      ...
      if (qTrans?.id) {
        await api.permanentDeleteOrderingQuestionTranslation(qTrans.id);
      }
      for (const opt of allOptions) {
        const optTrans = (opt.translations || []).find((t: any) => t.language_id === langId);
        if (optTrans?.id) {
          await api.permanentDeleteOrderingItemTranslation(optTrans.id);
        }
      }
      ...
      if (r.success && r.data) {
        ...
        setAllOptions(r.data.items || []);
        setTranslationCoverage(r.data.translation_coverage || []);
      }
      if (editLangId === langId) {
        setEditLangId(7);
        loadLanguageIntoForm(7, r?.data?.question_translations || allTranslations, r?.data?.items || allOptions);
      }
    } catch (e: any) {
  ...
  // ── Delete ALL non-English translations ──
  const handleDeleteAllTranslations = async () => {
    if (!editingId) return;
    const nonEngLangs = translationCoverage.filter(c => c.language_id !== 7 && (c.has_question_translation || c.has_item_translations));
  ...
  const handleSoftDelete = async (id: number) => {
    ...
    try {
      const r = await api.deleteOrderingQuestion(id);
      if (r.success) {
  ...
  const handleRestore = async (id: number) => {
    setActionLoadingId(id);
    try {
      const r = await api.restoreOrderingQuestion(id);
      if (r.success) {
  ...
  const handlePermanentDelete = async (id: number) => {
    ...
    setActionLoadingId(id);
    try {
      const r = await api.permanentDeleteOrderingQuestion(id);
      if (r.success) {
  ...
  const translatedLangs = translationCoverage.filter(c => c.has_question_translation && c.has_item_translations).length;
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Create, manage, and translate ordering questions with items in one place"
        actions={
          mode === 'list' && !showTrash ? (
        ...
        <div className="space-y-4">
          {listLoading ? (
            ...
            <EmptyState
              ...
              description={showTrash ? 'No deleted questions' : (searchDebounce || filterDifficulty || filterStatus ? 'No questions match your filters' : topicId ? 'Add your first ordering question' : 'Select a subject, chapter, and topic to create questions — or browse all questions below')}
                ...
                <TBody>
                  {questions.map(q => (
                    <TR key={q.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(q.id) && 'bg-brand-50/40')}>
                      ...
                      <TD className="py-2.5 text-center">{q.item_count}</TD>
                      <TD className="py-2.5">
                        {!showTrash ? (
            ...
            <div className="px-6 pt-3 border-b bg-gray-50">
              <div className="flex gap-1 overflow-x-auto">
                {availableLanguages.map(lang => {
                  ...
                  const hasTranslation = coverage?.has_question_translation && coverage?.has_item_translations;
                  return (
                    <button
                      key={lang.id}
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="block text-sm font-medium text-gray-700">
                  Ordering Items ({currentLangName}) *
                  <span className="text-xs text-gray-400 ml-2">
                    (items will be shown in random order, correct position = the answer)
                  </span>
                </label>
                {!isNonEnglish && (
              ...
              <div className="space-y-2">
                {items.map((item, idx) => (
                  <div key={idx} className="flex items-center gap-2">
                    <Badge className="flex-shrink-0 bg-gray-100 text-gray-600 text-xs font-mono w-8 h-8 flex items-center justify-center rounded-full">
                      #{idx + 1}
                    </Badge>
                    <Input
                      className="flex-1"
                      ...
                      placeholder={`Item ${idx + 1} in ${currentLangName}...`}
                    />
                    <div className="flex-shrink-0 w-24">
                      <Input
                        ...
                        onChange={e => updateItemPosition(idx, parseInt(e.target.value) || 1)}
                        title="Correct position"
                        disabled={isNonEnglish}
                        className={cn(isNonEnglish && "bg-gray-100 text-gray-500")}
                      />
                    </div>
                    {items.length > 2 && !isNonEnglish && (
                      <button
              ...
              {isNonEnglish && (
                <p className="text-xs text-gray-400 mt-2">
                  Correct position and adding/removing items can only be done in English.
                </p>
              )}
            </div>
              ...
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                <div className="flex flex-wrap gap-2">
                  {translationCoverage.map(c => {
                    const complete = c.has_question_translation && c.has_item_translations;
                    const partial = c.has_question_translation || c.has_item_translations;
                    return (
                      <Badge
                        key={c.language_id}

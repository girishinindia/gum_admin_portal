...
export default function OwQuestionsPage() {
  ...
  async function loadCoverage() {
    ...
    if (res.success && Array.isArray(res.data)) {
      ...
      res.data.forEach((c: CoverageItem) => { map[c.ow_question_id] = c; });
      setCoverage(map);
    }
  }
            ...
            <TBody>
              {items.map(c => (
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    <button
                      onClick={() => router.push(`/ow-synonyms?ow_question_id=${c.id}`)}
                  ...
                  {!showTrash && (
                    <TD className="py-2.5">
                      {(() => {
                        ...
                        if (!cov) return (
                          <button
                            onClick={() => router.push(`/ow-question-translations?ow_question_id=${c.id}`)}

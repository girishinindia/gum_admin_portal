...
interface McqQuestionTranslation {
  id: number;
  ...
  created_at: string;
  updated_at: string;
  mcq_questions?: { code: string; slug?: string } | null;
...
type SortField = 'id' | 'created_at' | 'mcq_question_id' | 'language_id';
...
export default function McqQuestionTranslationsPage() {
  const [items, setItems] = useState<McqQuestionTranslation[]>([]);
  const [mcqQuestions, setMcqQuestions] = useState<McqQuestion[]>([]);
  const [languages, setLanguages] = useState<Language[]>([]);
  const [coverage, setCoverage] = useState<CoverageItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<McqQuestionTranslation | null>(null);
  ...
  const [viewItem, setViewItem] = useState<McqQuestionTranslation | null>(null);

  const toolbarRef = useRef<DataToolbarHandle>(null);
  const router = useRouter();

  // --- Auto-fetch translation when question or language changes ---
  const watchedQuestionId = watch('mcq_question_id');
  ...
  useEffect(() => {
    ...
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?mcq_question_id=${watchedQuestionId}&language_id=${watchedLangId}&limit=1`;
      const res = await api.listMcqQuestionTranslations(qs);
      if (cancelled) return;

      if (res.success && res.data && res.data.length > 0) {
        const item = res.data[0] as McqQuestionTranslation;
        setEditing(item);
  ...
  useEffect(() => {
    api.listMcqQuestions('?limit=999&sort=display_order&order=asc').then(res => { if (res.success) setMcqQuestions(res.data || []); });
  ...
  useEffect(() => {
    setFilterQuestion('');
    if (filterTopicId) {
      api.listMcqQuestions(`?topic_id=${filterTopicId}&limit=999&sort=display_order&order=asc`).then(res => { if (res.success) setMcqQuestions(res.data || []); });
    } else {
      api.listMcqQuestions('?limit=999&sort=display_order&order=asc').then(res => { if (res.success) setMcqQuestions(res.data || []); });
    }
  }, [filterTopicId]);
  ...
  async function refreshSummary() {
    const res = await api.getTableSummary('mcq_question_translations');
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterQuestion) qs.set('mcq_question_id', filterQuestion);
      if (filterLanguage) qs.set('language_id', filterLanguage);
      if (filterStatus) qs.set('is_active', filterStatus);
    }
    qs.set('sort', sortField);
    qs.set('order', sortOrder);
    const res = await api.listMcqQuestionTranslations('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  const defaultFormValues = {
    mcq_question_id: '', language_id: '',
    question_text: '', hint_text: '', explanation_text: '',
    is_active: true,
    image_1: '', image_2: '',
  };

  function clearFormFields() {
    const qid = getValues('mcq_question_id');
    const lid = getValues('language_id');
    Object.entries(defaultFormValues).forEach(([k, v]) => setValue(k, v));
    setValue('mcq_question_id', qid);
    setValue('language_id', lid);
    resetImageState();
  }

  function populateForm(item: McqQuestionTranslation) {
    setValue('question_text', item.question_text || '');
    setValue('explanation', (item as any).explanation_text || (item as any).explanation || '');
    setValue('hint', (item as any).hint_text || (item as any).hint || '');
    setValue('is_active', item.is_active ?? true);
    setValue('image_1', item.image_1 || '');
    setValue('image_2', item.image_2 || '');
    resetImageState();
  }
  ...
  function openCreate() {
    ...
    reset({ ...defaultFormValues, mcq_question_id: mcqQuestions[0]?.id || '', language_id: languages[0]?.id || '' });
    setDialogOpen(true);
  }

  function openEdit(item: McqQuestionTranslation) {
    skipAutoFetchRef.current = true;
    setEditing(item); setFormMode('existing'); resetImageState(); setDialogKey(k => k + 1);
    reset({
      mcq_question_id: item.mcq_question_id, language_id: item.language_id,
      question_text: item.question_text || '', hint_text: item.hint_text || '', explanation_text: item.explanation_text || '',
      is_active: item.is_active ?? true,
      image_1: item.image_1 || '', image_2: item.image_2 || '',
    });
    setDialogOpen(true);
  }
  ...
  async function onSubmit(data: any) {
    ...
    // Scalar text fields
    const scalarFields = [
      'mcq_question_id', 'language_id', 'question_text', 'hint_text', 'explanation_text',
      'image_1', 'image_2',
    ];
    ...
    const res = editing
      ? await api.updateMcqQuestionTranslation(editing.id, fd, true)
      : await api.createMcqQuestionTranslation(fd, true);
    if (res.success) {
  ...
  async function handleBulkSoftDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.deleteMcqQuestionTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkRestore() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.restoreMcqQuestionTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.permanentDeleteMcqQuestionTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="MCQ Question Translations" description="Manage translations for MCQ questions"
        ...
        {!showTrash && (
          <>
            ...
            <select className={selectClass} value={filterQuestion} onChange={e => setFilterQuestion(e.target.value)} disabled={!filterTopicId}>
              ...
              {mcqQuestions.map(q => <option key={q.id} value={q.id}>{q.display_order ? q.display_order + '. ' : ''}{q.code}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : Languages} title={showTrash ? 'Trash is empty' : 'No translations yet'} description={showTrash ? 'Deleted translations will appear here' : 'Add your first MCQ question translation'}
            ...
            <Table>
              <THead>
                <TR className="hover:bg-transparent">
                  ...
                  <TH><button onClick={() => handleSort('mcq_question_id')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">Question <SortIcon field="mcq_question_id" /></button></TH>
              ...
              <TBody>
                {items.map(item => (
                  <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                    ...
                    <TD className="py-2.5">
                      {item.mcq_questions?.code ? (
                        <div>
                          <Badge variant="info">{item.mcq_questions.code}</Badge>
                          {item.mcq_questions.slug && <span className="block text-xs text-slate-400 mt-0.5">{item.mcq_questions.slug}</span>}
                        </div>
                    ...
                    <TD className="py-2.5">
                      <span className={cn('text-sm', showTrash ? 'text-slate-500 line-through' : 'text-slate-900')}>
                        {item.question_text ? (item.question_text.length > 60 ? item.question_text.substring(0, 60) + '...' : item.question_text) : <span className="text-slate-300 italic">Empty</span>}
                      </span>
                    </TD>
                    <TD className="py-2.5">
                      <div className="flex items-center gap-1">
                        {item.image_1 && <span title="Image 1"><ImageIcon className="w-3.5 h-3.5 text-emerald-500" /></span>}
                        {item.image_2 && <span title="Image 2"><ImageIcon className="w-3.5 h-3.5 text-blue-500" /></span>}
                        {!item.image_1 && !item.image_2 && <span className="text-slate-300">--</span>}
                      </div>
                    </TD>
      ...
      {/* --- View Dialog --- */}
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} title="View MCQ Question Translation" size="lg">
        {viewItem && (
          <div className="p-6 space-y-4">
            {/* Header info */}
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{mcqQuestions.find(q => q.id === viewItem.mcq_question_id)?.code || `Question #${viewItem.mcq_question_id}`}</Badge>
            ...
            <div className="space-y-3">
              ...
              <ViewField label="Explanation" value={viewItem.explanation_text} />
              <ViewField label="Hint" value={viewItem.hint_text} />
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  {viewItem.image_1 ? (
                    <img src={viewItem.image_1} alt="Image 1" className="max-h-40 rounded-lg border border-slate-200" />
                  ) : (
                    <p className="text-sm text-slate-300 italic">Not set</p>
                  )}
                </div>
                <div>
                  ...
                  {viewItem.image_2 ? (
                    <img src={viewItem.image_2} alt="Image 2" className="max-h-40 rounded-lg border border-slate-200" />
                  ) : (
                    <p className="text-sm text-slate-300 italic">Not set</p>
                  )}
                </div>
      ...
      {/* --- Edit / Create Dialog --- */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit MCQ Question Translation' : 'Add MCQ Question Translation'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getMcqQuestionTranslation(editing.id);
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">MCQ Question</label>
              <select className={cn(selectClass, 'w-full')} {...register('mcq_question_id', { required: true })}>
                {mcqQuestions.map(q => <option key={q.id} value={q.id}>{q.display_order ? q.display_order + '. ' : ''}{q.code}</option>)}
              </select>
            </div>
            <div>
          ...
          <div>
            ...
            <textarea className={cn(selectClass, 'w-full min-h-[80px]')} placeholder="Explanation for the correct answer..." {...register('explanation_text')} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <ImageUpload key={`img1-${dialogKey}`} label="Image 1" hint="Question image 1"
              value={editing?.image_1} shape="rounded"
            ...
            <ImageUpload key={`img2-${dialogKey}`} label="Image 2" hint="Question image 2"
              value={editing?.image_2} shape="rounded"

...
export default function McqQuestionTranslationsPage() {
  ...
  useEffect(() => {
    api.listMcqQuestions('?limit=999&sort=code&order=asc').then(res => { if (res.success) setMcqQuestions(res.data || []); });
        ...
        {!showTrash && (
          <>
            <select className={selectClass} value={filterQuestion} onChange={e => setFilterQuestion(e.target.value)}>
              ...
              {mcqQuestions.map(q => <option key={q.id} value={q.id}>{q.code}{q.slug ? ` (${q.slug})` : ''}</option>)}
            </select>

...
interface MiniProjectListItem {
  ...
  chapters?: { name: string; slug: string; subject_id: number; subjects?: { name: string; slug: string } } | null;
...
export default function MiniProjectsPage() {
  ...
  // ── List state ──
  const [projects, setProjects] = useState<MiniProjectListItem[]>([]);
  ...
  const loadSummary = useCallback(async () => {
    try {
      const res = await api.getTableSummary('assesment_mini_projects');
  ...
  const loadProjects = useCallback(async () => {
    ...
    try {
      ...
      const r = await api.listMiniProjects('?' + qs.toString());
      if (r.success) {
        setProjects(r.data || []);
  ...
  const startEdit = async (id: number) => {
    try {
      const r = await api.getMiniProjectFull(id);
      if (!r.success || !r.data) { toast.error('Failed to load mini project'); return; }
      const p = r.data;
      setEditingId(id);
      setFormChapterId(p.chapter_id || '');
      ...
      // Pre-populate cascade selects
      setFormSubjects(subjects);
      setFormSubjectId('');
      setFormChapters([]);

      // Load solutions
      const solR = await api.listMiniProjectSolutions(`?mini_project_id=${id}&limit=50`);
  ...
  const handleSaveTranslation = async () => {
    ...
    try {
      ...
      if (existingTrans && existingTrans.id) {
        await api.updateMiniProjectTranslation(existingTrans.id, data, fileHtml || undefined);
      } else {
        data.mini_project_id = editingId;
        data.language_id = editLangId;
        await api.createMiniProjectTranslation(data, fileHtml || undefined);
      }
      ...
      const r = await api.getMiniProjectFull(editingId);
      if (r.success && r.data) {
        setAllTranslations(r.data.assesment_mini_projects_translations || []);
        setTranslationCoverage(r.data.translation_coverage || []);
        loadLanguageIntoForm(editLangId, r.data.assesment_mini_projects_translations || []);
      }
      loadProjects();
    } catch (e: any) {
  ...
  const handleSave = async () => {
    ...
    if (!formChapterId && mode === 'create') { toast.error('Please select a chapter'); return; }
    if (!transTitle.trim()) { toast.error('Name is required'); return; }
    ...
    try {
      const payload: any = {
        name: transTitle.trim(),
        chapter_id: formChapterId || undefined,
      ...
      if (mode === 'edit' && editingId) {
        r = await api.updateFullMiniProject(editingId, payload, fileHtml || undefined, fileSolution || undefined);
      } else {
        r = await api.createFullMiniProject(payload, fileHtml || undefined, fileSolution || undefined);
      }
  ...
  const handleDeleteTranslation = async (langId: number) => {
    ...
    try {
      ...
      if (trans?.id) {
        await api.deleteMiniProjectTranslation(trans.id);
      }
      ...
      if (editLangId === langId) {
        setEditLangId(7);
        loadLanguageIntoForm(7, r?.data?.assesment_mini_projects_translations || allTranslations);
      }
    } catch (e: any) {
  ...
  const handleSoftDelete = async (id: number) => {
    ...
    try {
      const r = await api.softDeleteMiniProject(id);
      if (r.success) {
  ...
  const handleRestore = async (id: number) => {
    setActionLoadingId(id);
    try {
      const r = await api.restoreMiniProject(id);
      if (r.success) {
  ...
  const handlePermanentDelete = async (id: number) => {
    ...
    setActionLoadingId(id);
    try {
      const r = await api.deleteMiniProject(id);
      if (r.success) {
  ...
  const handleBulkSoftDelete = async () => {
    ...
    for (let i = 0; i < ids.length; i++) {
      const res = await api.softDeleteMiniProject(ids[i]);
      if (res.success) ok++;
  ...
  const handleBulkRestore = async () => {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.restoreMiniProject(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  const handleBulkPermanentDelete = async () => {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.deleteMiniProject(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  const openView = async (id: number) => {
    ...
    try {
      ...
      if (r.success && r.data) {
        ...
        toast.error('Failed to load mini project');
      }
    } catch {
      toast.error('Failed to load mini project');
    } finally {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Mini Projects"
        description="Create, manage, and translate mini projects with file uploads and solution videos"
        actions={
          mode === 'list' && !showTrash ? (
      ...
      {mode === 'list' && summary && (
        <div className="grid grid-cols-4 gap-4 mb-5">
          {[
            { label: 'Total Mini Projects', value: summary.total, icon: BarChart3, color: 'bg-blue-50 text-blue-600' },
        ...
        <div className="space-y-4">
          {listLoading ? (
            ...
            <EmptyState
              ...
              description={showTrash ? 'No deleted mini projects' : (searchDebounce || filterDifficulty || filterStatus ? 'No mini projects match your filters' : 'Add your first mini project')}
              action={!showTrash && !searchDebounce && !filterDifficulty && !filterStatus ? <Button onClick={startCreate}><Plus className="w-4 h-4" /> Add Mini Project</Button> : undefined}
            />
          ) : (
        ...
        <div className="bg-white border rounded-lg">
          ...
          {mode === 'create' && (
            <div className="px-4 py-3 border-b bg-slate-50 flex flex-wrap items-center gap-2">
              ...
              <select className="h-8 px-2 pr-6 text-xs rounded border border-slate-200 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 appearance-none cursor-pointer" value={formChapterId} onChange={e => setFormChapterId(e.target.value ? parseInt(e.target.value) : '')} disabled={!formSubjectId}>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-4">
                <h4 className="text-sm font-semibold text-gray-700 border-b pb-2">Mini Project Details</h4>

                {/* Chapter display in edit mode */}
                {mode === 'edit' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Chapter</label>
                    <div className="px-3 py-2 bg-gray-50 border rounded-md text-sm text-gray-600">
                      {formChapterId ? `ID: ${formChapterId}` : '---'}
                    </div>
                    <p className="text-xs text-gray-400 mt-1">Chapter cannot be changed after creation</p>
                  </div>
                )}
      ...
      {/* ── View Dialog ── */}
      <Dialog open={viewDialogOpen} onClose={() => { setViewDialogOpen(false); setViewProject(null); }} title="Mini Project Details" size="lg">
        {viewLoading ? (
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-3 mb-6">
              ...
              <DetailRow label="Subject" value={viewProject.chapters?.subjects?.name} />
              <DetailRow label="Chapter" value={viewProject.chapters?.name || viewProject.chapters?.slug} />
            ...
            {viewProject.translation_coverage && viewProject.translation_coverage.length > 0 && (
              <div className="border-t pt-4">
                ...
                {(() => {
                  const trans = (viewProject.assesment_mini_projects_translations || []).find((t: any) => t.language_id === viewLangId);

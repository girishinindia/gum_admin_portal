...
export default function MiniProjectsPage() {
  ...
  useEffect(() => { setPage(1); }, [searchDebounce, filterSubject, filterChapter, filterStatus, filterDifficulty, pageSize, showTrash]);

  // Load data
  useEffect(() => { load(); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterSubject, filterChapter, filterStatus, filterDifficulty, showTrash]);
  ...
  async function loadSummary() {
    const res: any = await api.listAssessments('?assessment_type=mini_project&summary=true');
  ...
  async function handleAiGenerate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.success(`AI generated ${count} mini project(s)`);
        load();
        loadSummary();
      } else {
  ...
  async function handleAiTranslateAll() {
    if (!filterChapter || items.length === 0) { toast.error('Select a chapter with mini projects first'); return; }
  ...
  function openEdit(item: Assessment) {
    setEditing(item);
    setDialogKey(k => k + 1);
    // Pre-populate dialog cascade from item's chapter relationship
    if (item.chapter_id) {
      api.listChapters(`?limit=1&id=${item.chapter_id}`).then(async (res) => {
    ...
    reset({
      chapter_id: item.chapter_id ? String(item.chapter_id) : '',
  ...
  async function onSubmit(data: any) {
    ...
    if (res.success) {
      toast.success(editing ? 'Mini Project updated' : 'Mini Project created');
      setDialogOpen(false);
      load();
      loadSummary();
    } else {
  ...
  async function onSoftDelete(item: Assessment) {
    ...
    if (res.success) { toast.success('Mini Project moved to trash'); load(); loadSummary(); }
  ...
  async function onRestore(item: Assessment) {
    ...
    setActionLoadingId(null);
    if (res.success) { toast.success('Mini Project restored'); load(); loadSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  async function onPermanentDelete(item: Assessment) {
    ...
    setActionLoadingId(null);
    if (res.success) { toast.success('Mini Project permanently deleted'); load(); loadSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Mini Projects"
        description="Manage mini project assessments"
        actions={
            ...
            {!showTrash && (
              <>
                <button
                  ...
                  title={filterChapter ? 'AI Generate Mini Projects' : 'Select a chapter first'}
                  className={cn(
                ...
                <button
                  ...
                  disabled={!filterChapter || aiTranslating || items.length === 0}
                  title={filterChapter ? 'AI Translate all mini projects in this chapter' : 'Select a chapter first'}
                  className={cn(
                    'inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg transition-colors',
                    filterChapter && !aiTranslating && items.length > 0
                ...
                <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add Mini Project</Button>
              </>
            )}
          </div>
        }
      ...
      <DataToolbar ref={toolbarRef} search={search} onSearchChange={setSearch} searchPlaceholder={showTrash ? 'Search trash...' : 'Search mini projects...'}>
        {!showTrash && (
          <>
            <SearchableSelect
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : FileQuestion} title={showTrash ? 'Trash is empty' : 'No mini projects yet'}
          description={showTrash ? 'No deleted mini projects' : (searchDebounce || filterChapter || filterStatus || filterDifficulty ? 'No mini projects match your filters' : 'Add your first mini project')}
          action={!showTrash && !searchDebounce && !filterChapter && !filterStatus && !filterDifficulty ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add Mini Project</Button> : undefined} />
      ) : (
                ...
                <TR key={item.id} className={showTrash ? 'bg-amber-50/30' : undefined}>
                  ...
                  <TD className="py-2.5">
                    <span className="text-sm text-slate-600">
                      {item.chapter?.english_name || item.chapter?.slug || (item.chapter_id ? `ID: ${item.chapter_id}` : '—')}
                    </span>
                  </TD>
                  <TD className="py-2.5">
      ...
      {/* View Dialog */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Mini Project Details" size="md">
        {viewing && (
          <div className="p-6">
            <div className="flex items-center gap-4 mb-6">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">{viewing.english_title || viewing.title || `Mini Project #${viewing.id}`}</h3>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              <DetailRow label="Chapter" value={viewing.chapter?.english_name || viewing.chapter?.slug || (viewing.chapter_id ? `ID: ${viewing.chapter_id}` : undefined)} />
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-slate-900">AI Generate Mini Projects</h3>
              </div>
            ...
            <form onSubmit={handleAiGenerate} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Number of Mini Projects <span className="text-slate-400 font-normal">(0 = AI decides)</span></label>
              ...
              {aiResult && (
                <div className={cn('px-3 py-2 rounded-lg text-sm', aiResult.success ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200')}>
                  {aiResult.success ? `Successfully generated ${aiResult.count} mini project(s)!` : aiResult.error}
                </div>
              )}
      ...
      {/* Create / Edit Dialog */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Mini Project' : 'Add Mini Project'} size="md">
        <form key={dialogKey} onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          {/* Cascade: Subject → Chapter */}
          <SearchableSelect
            label="Subject"
            options={subjects.map(s => ({ value: String(s.id), label: s.english_name || '' }))}
            value={dialogSubject}

...
import { Plus, FolderOpen, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3, RotateCcw, AlertTriangle, X, Sparkles, Loader2 } from 'lucide-react';
import { cn, fromNow } from '@/lib/utils';
import type { DocumentType } from '@/lib/types';

type SortField = 'id' | 'name' | 'sort_order' | 'is_active';
...
export default function DocumentTypesPage() {
  ...
  useEffect(() => {
    api.getTableSummary('document_types').then(res => {
  ...
  async function refreshSummary() {
    const res = await api.getTableSummary('document_types');
    if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
  }
  ...
  function openView(dt: DocumentType) {
    setViewing(dt);
  }
  ...
  async function onSoftDelete(dt: DocumentType) {
    if (!confirm(`Move "${dt.name}" to trash? You can restore it later.`)) return;
    setActionLoadingId(dt.id);
    const res = await api.deleteDocumentType(dt.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Document type moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(dt: DocumentType) {
    setActionLoadingId(dt.id);
    const res = await api.restoreDocumentType(dt.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`"${dt.name}" restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(dt: DocumentType) {
    if (!confirm(`PERMANENTLY delete "${dt.name}"? This cannot be undone.`)) return;
    setActionLoadingId(dt.id);
    const res = await api.permanentDeleteDocumentType(dt.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Document type permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(dt: DocumentType) {
    const res = await api.updateDocumentType(dt.id, { is_active: !dt.is_active });
    if (res.success) { toast.success(`${!dt.is_active ? 'Activated' : 'Deactivated'}`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  async function handleBulkSoftDelete() {
    ...
    try {
      for (let i = 0; i < ids.length; i++) {
        await api.deleteDocumentType(ids[i]);
  ...
  async function handleBulkRestore() {
    ...
    try {
      for (let i = 0; i < ids.length; i++) {
        await api.restoreDocumentType(ids[i]);
        setBulkProgress({ done: i + 1, total: ids.length });
      }
  ...
  async function handleBulkPermanentDelete() {
    ...
    try {
      for (let i = 0; i < ids.length; i++) {
        await api.permanentDeleteDocumentType(ids[i]);
        setBulkProgress({ done: i + 1, total: ids.length });
      }
    ...
    <div className="animate-fade-in">
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-4 mb-5">
          {[
            { label: 'Total Document Types', value: summary.total, icon: BarChart3, color: 'bg-blue-50 text-blue-600' },
      ...
      <DataToolbar
        ...
        searchPlaceholder={showTrash ? 'Search trash...' : 'Search document types...'}
      >
        {!showTrash && (
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={showTrash ? 'Trash is empty' : 'No document types yet'}
          description={showTrash ? 'No deleted document types' : (searchDebounce || filterStatus ? 'No document types match your filters' : 'Add your first document type')}
          action={!showTrash && !searchDebounce && !filterStatus ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add type</Button> : undefined}
        />
      ) : (
        <div className={cn('mt-4 bg-white rounded-xl border overflow-hidden shadow-sm', showTrash ? 'border-amber-200' : 'border-slate-200')}>
            ...
            <TBody>
              {types.map(dt => (
                <TR key={dt.id} className={cn(selectedIds.has(dt.id) ? 'bg-brand-50/40' : showTrash ? 'bg-amber-50/30' : undefined)}>
                  ...
                  {!showTrash && (
                    <TD className="py-2.5">
                      <span className="text-slate-600 text-sm line-clamp-1">{dt.description || '—'}</span>
                    </TD>
                  )}
                  <TD className="py-2.5">
                    <span className="text-slate-600">{dt.sort_order}</span>
                  </TD>
                  {showTrash && (
                    <TD className="py-2.5">
                      <span className="text-xs text-amber-600">{dt.deleted_at ? fromNow(dt.deleted_at) : '—'}</span>
                    </TD>
                  )}
                  <TD className="py-2.5">
                    {showTrash ? (
                      ...
                      <Badge variant={dt.is_active ? 'success' : 'danger'}>
                  ...
                  <TD className="py-2.5">
                    <div className="flex items-center justify-end gap-1">
                      {showTrash ? (
                        <>
                          <button onClick={() => onRestore(dt)} disabled={actionLoadingId !== null} className="p-1.5 rounded-md text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 transition-colors disabled:opacity-50" title="Restore">
                            {actionLoadingId === dt.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RotateCcw className="w-3.5 h-3.5" />}
                          </button>
                          <button onClick={() => onPermanentDelete(dt)} disabled={actionLoadingId !== null} className="p-1.5 rounded-md text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50" title="Delete permanently">
                            {actionLoadingId === dt.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                          </button>
                        </>
                      ) : (
                        <>
                          <button onClick={() => openView(dt)} className="p-1.5 rounded-md text-slate-400 hover:text-sky-600 hover:bg-sky-50 transition-colors" title="View">
                            <Eye className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => openEdit(dt)} className="p-1.5 rounded-md text-slate-400 hover:text-brand-600 hover:bg-brand-50 transition-colors" title="Edit">
                            <Edit2 className="w-3.5 h-3.5" />
                          </button>
                          <button onClick={() => onSoftDelete(dt)} disabled={actionLoadingId !== null} className="p-1.5 rounded-md text-slate-400 hover:text-red-600 hover:bg-red-50 transition-colors disabled:opacity-50" title="Move to Trash">
                            {actionLoadingId === dt.id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Trash2 className="w-3.5 h-3.5" />}
                          </button>
                        </>
                      )}
          ...
          <Pagination
            ...
            total={total}
            showingCount={types.length}
          />
        </div>
      )}

      {/* ── View Document Type Dialog ── */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Document Type Details" size="md">
        {viewing && (
          <div className="p-6">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {editing.is_active ? 'This document type is currently active' : 'This document type is currently inactive'}
                </p>
              </div>
              <button
                type="button"
                onClick={async () => {
                  ...
                  const refreshed = await api.getDocumentType(editing.id);

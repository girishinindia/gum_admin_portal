...
import { Plus, FileEdit, Trash2, Edit2, Eye, ArrowUpDown, ArrowUp, ArrowDown, CheckCircle2, XCircle, BarChart3, RotateCcw, AlertTriangle, Loader2, Check, X } from 'lucide-react';
...
const ANSWER_TYPE_COLORS: Record<string, string> = {
  ...
  long_answer: 'bg-blue-50 text-blue-700',
};
...
export default function DescQuestionsPage() {
  ...
  useEffect(() => {
    api.getTableSummary('descriptive_questions').then(res => {
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterStatus, filterSubjectId, filterChapterId, filterTopicId, filterAnswerType, filterDifficulty, pageSize, showTrash]);
  useEffect(() => { load(); loadCoverage(); setSelectedIds(new Set()); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterStatus, filterSubjectId, filterChapterId, filterTopicId, filterAnswerType, filterDifficulty, showTrash]);
  ...
  async function load() {
    ...
    const res = await api.listDescQuestions('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  function openCreate() {
    ...
    reset({
      topic_id: '', code: '', slug: '', answer_type: 'short_answer',
  ...
  async function openEdit(c: any) {
    ...
    reset({
      ...
      answer_type: c.answer_type || 'short_answer',
  ...
  async function onSubmit(data: any) {
    ...
    const res = editing
      ? await api.updateDescQuestion(editing.id, payload)
      : await api.createDescQuestion(payload);
    if (res.success) {
      toast.success(editing ? 'Descriptive question updated' : 'Descriptive question created');
  ...
  async function onSoftDelete(c: any) {
    ...
    const res = await api.deleteDescQuestion(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Descriptive question moved to trash'); load(); refreshSummary(); loadCoverage(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(c: any) {
    setActionLoadingId(c.id);
    const res = await api.restoreDescQuestion(c.id);
    setActionLoadingId(null);
  ...
  async function onPermanentDelete(c: any) {
    ...
    setActionLoadingId(c.id);
    const res = await api.permanentDeleteDescQuestion(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Descriptive question permanently deleted'); load(); refreshSummary(); loadCoverage(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(c: any) {
    const res = await api.updateDescQuestion(c.id, { is_active: !c.is_active });
  ...
  function formatAnswerType(val: string) {
    const map: Record<string, string> = { short_answer: 'Short Answer', long_answer: 'Long Answer' };
    return map[val] || val;
  }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Manage short answer and long answer descriptive questions"
        actions={
        ...
        {!showTrash && (
          <>
            ...
            <select className={selectClass} value={filterAnswerType} onChange={e => setFilterAnswerType(e.target.value)}>
              ...
              {ANSWER_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={showTrash ? Trash2 : FileEdit}
          title={showTrash ? 'Trash is empty' : 'No descriptive questions yet'}
          description={showTrash ? 'No deleted questions' : (searchDebounce || filterStatus || filterTopicId || filterAnswerType || filterDifficulty ? 'No questions match your filters' : 'Add your first descriptive question')}
          action={!showTrash && !searchDebounce && !filterStatus && !filterTopicId && !filterAnswerType && !filterDifficulty ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add question</Button> : undefined}
        />
      ) : (
              ...
              <TR className="hover:bg-transparent">
                ...
                <TH>
                  <button onClick={() => handleSort('answer_type')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">
                    Answer Type <SortIcon field="answer_type" />
                  </button>
                </TH>
                <TH>
            ...
            <TBody>
              {items.map(c => (
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    {c.answer_type ? (
                      <span className={cn('inline-flex text-xs font-semibold px-2 py-0.5 rounded-full', ANSWER_TYPE_COLORS[c.answer_type] || 'bg-slate-50 text-slate-600')}>
                        {formatAnswerType(c.answer_type)}
                      </span>
      ...
      {/* ── View Descriptive Question Dialog ── */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Descriptive Question Details" size="lg">
        {viewing && (
          <div className="p-6">
            {/* Header */}
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {viewing.answer_type && (
                    <span className={cn('inline-flex text-xs font-semibold px-2 py-0.5 rounded-full', ANSWER_TYPE_COLORS[viewing.answer_type] || 'bg-slate-50 text-slate-600')}>
                      {formatAnswerType(viewing.answer_type)}
                    </span>
                  )}
            ...
            <div className="grid grid-cols-3 gap-x-8 gap-y-4">
              ...
              <DetailRow label="Answer Type" value={viewing.answer_type ? formatAnswerType(viewing.answer_type) : undefined} />
            ...
            <div className="mt-6 pt-4 border-t border-slate-100">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                {!viewing.is_mandatory && !viewing.is_active && (
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getDescQuestion(editing.id);
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input label="Code" placeholder="desc-math-001" {...register('code', { required: true })} />
              <Input label="Slug" placeholder="desc-math-001" {...register('slug', { required: true, onChange: () => setSlugManual(true) })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">Answer Type</label>
                <select className={cn(selectClass, 'w-full')} {...register('answer_type')}>
                  {ANSWER_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>

...
type SortField = 'id' | 'code' | 'slug' | 'display_order' | 'mcq_type' | 'difficulty_level' | 'points' | 'is_active';
...
export default function McqQuestionsPage() {
  ...
  const [filterMcqType, setFilterMcqType] = useState<string>('');
  ...
  useEffect(() => {
    api.getTableSummary('mcq_questions').then(res => {
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterStatus, filterSubjectId, filterChapterId, filterTopicId, filterMcqType, filterDifficulty, pageSize, showTrash]);
  useEffect(() => { load(); loadCoverage(); setSelectedIds(new Set()); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterStatus, filterSubjectId, filterChapterId, filterTopicId, filterMcqType, filterDifficulty, showTrash]);
  ...
  async function load() {
    ...
    const res = await api.listMcqQuestions('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  async function loadCoverage() {
    ...
    if (res.success && Array.isArray(res.data)) {
      ...
      res.data.forEach((c: CoverageItem) => { map[c.mcq_question_id] = c; });
      setCoverage(map);
    }
  }
  ...
  function openCreate() {
    ...
    reset({
      topic_id: '', code: '', slug: '', mcq_type: 'single_choice',
  ...
  async function openEdit(c: any) {
    ...
    reset({
      ...
      mcq_type: c.mcq_type || 'single_choice',
  ...
  async function onSubmit(data: any) {
    ...
    const res = editing
      ? await api.updateMcqQuestion(editing.id, payload)
      : await api.createMcqQuestion(payload);
    if (res.success) {
      toast.success(editing ? 'MCQ question updated' : 'MCQ question created');
  ...
  async function onSoftDelete(c: any) {
    ...
    const res = await api.deleteMcqQuestion(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('MCQ question moved to trash'); load(); refreshSummary(); loadCoverage(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(c: any) {
    setActionLoadingId(c.id);
    const res = await api.restoreMcqQuestion(c.id);
    setActionLoadingId(null);
  ...
  async function onPermanentDelete(c: any) {
    ...
    setActionLoadingId(c.id);
    const res = await api.permanentDeleteMcqQuestion(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('MCQ question permanently deleted'); load(); refreshSummary(); loadCoverage(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(c: any) {
    const res = await api.updateMcqQuestion(c.id, { is_active: !c.is_active });
  ...
  function formatMcqType(val: string) {
    const map: Record<string, string> = { single_choice: 'Single Choice', multiple_choice: 'Multiple Choice', true_false: 'True / False' };
    return map[val] || val;
  }
    ...
    <div className="animate-fade-in">
      ...
      >
        {!showTrash && (
          <>
            ...
            <select className={selectClass} value={filterMcqType} onChange={e => setFilterMcqType(e.target.value)}>
              <option value="">All Types</option>
              {MCQ_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={showTrash ? 'No deleted questions' : (searchDebounce || filterStatus || filterTopicId || filterMcqType || filterDifficulty ? 'No questions match your filters' : 'Add your first MCQ question')}
          action={!showTrash && !searchDebounce && !filterStatus && !filterTopicId && !filterMcqType && !filterDifficulty ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add question</Button> : undefined}
        />
      ) : (
          ...
          <Table>
            <THead>
              <TR className="hover:bg-transparent">
                ...
                <TH>Question</TH>
                <TH>
                  <button onClick={() => handleSort('mcq_type')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">
                    Type <SortIcon field="mcq_type" />
                  </button>
                </TH>
                <TH>
            ...
            <TBody>
              {items.map(c => (
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    {c.mcq_type ? (
                      <span className={cn('inline-flex text-xs font-semibold px-2 py-0.5 rounded-full', MCQ_TYPE_COLORS[c.mcq_type] || 'bg-slate-50 text-slate-600')}>
                        {formatMcqType(c.mcq_type)}
                      </span>
                  ...
                  <TD className="py-2.5">
                    <span className="text-sm text-slate-600">{c.option_count ?? '--'}</span>
                  </TD>
                  {!showTrash && (
                    <TD className="py-2.5">
                      {(() => {
      ...
      {/* ── View MCQ Question Dialog ── */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="MCQ Question Details" size="lg">
        {viewing && (
          <div className="p-6">
            {/* Header */}
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  {viewing.mcq_type && (
                    <span className={cn('inline-flex text-xs font-semibold px-2 py-0.5 rounded-full', MCQ_TYPE_COLORS[viewing.mcq_type] || 'bg-slate-50 text-slate-600')}>
                      {formatMcqType(viewing.mcq_type)}
                    </span>
                  )}
            ...
            <div className="grid grid-cols-3 gap-x-8 gap-y-4">
              ...
              <DetailRow label="MCQ Type" value={viewing.mcq_type ? formatMcqType(viewing.mcq_type) : undefined} />
              ...
              <DetailRow label="Options Count" value={viewing.option_count != null ? String(viewing.option_count) : undefined} />
      ...
      {/* ── Create / Edit Dialog ── */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit MCQ Question' : 'Add MCQ Question'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getMcqQuestion(editing.id);
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input label="Code" placeholder="q-math-001" {...register('code', { required: true })} />
              <Input label="Slug" placeholder="q-math-001" {...register('slug', { required: true, onChange: () => setSlugManual(true) })} />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1.5">MCQ Type</label>
                <select className={cn(selectClass, 'w-full')} {...register('mcq_type')}>
                  {MCQ_TYPE_OPTIONS.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
                </select>
              </div>
              <div>

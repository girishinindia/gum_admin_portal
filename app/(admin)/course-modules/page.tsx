...
type SortField = 'id' | 'name' | 'slug' | 'is_active' | 'display_order';

interface CourseModule {
  id: number; course_id: number; name: string | null; slug: string | null;
  display_order: number; view_count: number; is_active: boolean;
...
interface CoverageItem { course_module_id: number; translated_count: number; total_languages: number; }
...
export default function CourseModulesPage() {
  ...
  const [viewing, setViewing] = useState<CourseModule | null>(null);
  ...
  // Load summary + coverage + courses once on mount
  useEffect(() => {
    api.getTableSummary('course_modules').then(res => {
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterStatus, filterCourse, pageSize, showTrash]);
  useEffect(() => { load(); setSelectedIds(new Set()); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterStatus, filterCourse, showTrash]);

  async function loadCourses() {
    const res = await api.listCourses('?limit=500&sort=code&ascending=true');
  ...
  async function load() {
    ...
    const res = await api.listCourseModules('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  async function refreshSummary() {
    const res = await api.getTableSummary('course_modules');
    if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
  }
  ...
  function getCourseBadge(item: CourseModule) {
    if (item.courses) return item.courses.code || item.courses.name || item.courses.slug;
    const found = courses.find(c => c.id === item.course_id);
    return found ? found.code : `ID: ${item.course_id}`;
  }

  function openCreate() {
    setEditing(null); setDialogKey(k => k + 1); setSlugManual(false); setActiveTab('basic');
    reset({
      course_id: '', name: '', slug: '', display_order: 0,
    });
    setDialogOpen(true);
  }

  function openEdit(c: CourseModule) {
    setEditing(c); setDialogKey(k => k + 1); setSlugManual(true); setActiveTab('basic');
    reset({
      course_id: c.course_id ?? '', name: c.name || '', slug: c.slug || '',
      display_order: c.display_order ?? 0,
    });
    setDialogOpen(true);
  }

  function openView(c: CourseModule) {
    setViewing(c);
  }
  ...
  async function onSubmit(data: any) {
    ...
    Object.keys(data).forEach(k => {
      ...
      const numericFields = ['course_id', 'display_order'];
      if (numericFields.includes(k) && v !== '') { payload[k] = Number(v); return; }
      payload[k] = v;
    });

    const res = editing
      ? await api.updateCourseModule(editing.id, payload)
      : await api.createCourseModule(payload);
    if (res.success) {
      toast.success(editing ? 'Course module updated' : 'Course module created');
      setDialogOpen(false); load(); refreshSummary();
    } else toast.error(res.error || 'Failed');
  }

  async function onSoftDelete(c: CourseModule) {
    if (!confirm(`Move "${c.name || c.slug}" to trash? You can restore it later.`)) return;
    setActionLoadingId(c.id);
    const res = await api.deleteCourseModule(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Course module moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(c: CourseModule) {
    setActionLoadingId(c.id);
    const res = await api.restoreCourseModule(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`"${c.name || c.slug}" restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(c: CourseModule) {
    if (!confirm(`PERMANENTLY delete "${c.name || c.slug}"? This cannot be undone.`)) return;
    setActionLoadingId(c.id);
    const res = await api.permanentDeleteCourseModule(c.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Course module permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(c: CourseModule) {
    const res = await api.updateCourseModule(c.id, { is_active: !c.is_active });
  ...
  async function handleBulkSoftDelete() {
    ...
    for (let i = 0; i < ids.length; i++) {
      const res = await api.deleteCourseModule(ids[i]);
      if (res.success) ok++;
  ...
  async function handleBulkRestore() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.restoreCourseModule(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
  ...
  async function handleBulkPermanentDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) {
      const res = await api.permanentDeleteCourseModule(ids[i]);
      if (res.success) ok++;
      setBulkProgress({ done: i + 1, total: ids.length });
    }
    ...
    <div className="animate-fade-in">
      ...
      {summary && (
        <div className="grid grid-cols-4 gap-4 mb-5">
          {[
            { label: 'Total Modules', value: summary.total, icon: BarChart3, color: 'bg-blue-50 text-blue-600' },
      ...
      <DataToolbar
        ...
        searchPlaceholder={showTrash ? 'Search trash...' : 'Search course modules...'}
      >
        {!showTrash && (
          <>
            <select className={selectClass} value={filterCourse} onChange={e => setFilterCourse(e.target.value)}>
              <option value="">All Courses</option>
              {courses.map(c => <option key={c.id} value={String(c.id)}>{c.code}{c.name ? ` - ${c.name}` : ''}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          title={showTrash ? 'Trash is empty' : 'No course modules yet'}
          description={showTrash ? 'No deleted course modules' : (searchDebounce || filterStatus || filterCourse ? 'No course modules match your filters' : 'Add your first course module')}
          action={!showTrash && !searchDebounce && !filterStatus && !filterCourse ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add module</Button> : undefined}
        />
      ) : (
            ...
            <TBody>
              {items.map(c => (
                <TR key={c.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(c.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    <Badge variant="info">{getCourseBadge(c)}</Badge>
                  </TD>
                  <TD className="py-2.5">
                    <span className={cn('text-sm font-medium', showTrash ? 'text-slate-500 line-through' : 'text-slate-900')}>{c.english_name || c.name || ''}</span>
                  </TD>
                  <TD className="py-2.5">
                    <span className={cn('text-sm font-mono', showTrash ? 'text-slate-400 line-through' : 'text-slate-600')}>{c.slug || '--'}</span>
                  </TD>
                  <TD className="py-2.5">
                    <span className="text-sm text-slate-600">{c.display_order ?? '--'}</span>
                  </TD>
                  {!showTrash && (
                    <TD className="py-2.5">
                      {(() => {
      ...
      {/* ── View Course Module Dialog ── */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Course Module Details" size="lg">
        {viewing && (
          <div className="p-6">
            {/* Header */}
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <Badge variant="info">{getCourseBadge(viewing)}</Badge>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-x-8 gap-y-4">
              ...
              <DetailRow label="Course" value={getCourseBadge(viewing)} />
            ...
            <div className="mt-6 pt-4 border-t border-slate-100">
              ...
              <div className="grid grid-cols-3 gap-x-8 gap-y-4">
                <DetailRow label="View Count" value={viewing.view_count != null ? String(viewing.view_count) : undefined} />
              </div>
            </div>
      ...
      {/* ── Create / Edit Dialog ── */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Course Module' : 'Add Course Module'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              <div>
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  {editing.is_active ? 'This module is currently active' : 'This module is currently inactive'}
                </p>
              </div>
              <button
                type="button"
                onClick={async () => {
                  ...
                  const refreshed = await api.getCourseModule(editing.id);
          ...
          {activeTab === 'basic' && (
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <Input label="Name" placeholder="Module name" {...register('name', { required: true })} />
                <Input label="Slug" placeholder="module-name" {...register('slug', { required: true, onChange: () => setSlugManual(true) })} />
              </div>
              <div className="grid grid-cols-2 gap-3">
          ...
          {activeTab === 'stats' && !editing && (
            <p className="text-sm text-slate-500">Stats are only available when editing an existing course module.</p>
          )}

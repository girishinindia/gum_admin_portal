...
export default function TopicTranslationsPage() {
  const [items, setItems] = useState<TopicTranslation[]>([]);
  const [topics, setTopics] = useState<Topic[]>([]);
  const [languages, setLanguages] = useState<Language[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<TopicTranslation | null>(null);
  ...
  const MLI_FIELDS = ['tt-name', 'tt-short-intro', 'tt-long-intro', 'tt-prerequisites', 'tt-learning-objectives'];
  ...
  useEffect(() => {
    ...
    if (!watchedTopicId || !watchedLangId) return;

    let cancelled = false;
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?topic_id=${watchedTopicId}&language_id=${watchedLangId}&limit=1`;
      const res = await api.listTopicTranslations(qs);
      if (cancelled) return;

      if (res.success && res.data && res.data.length > 0) {
        const item = res.data[0] as TopicTranslation;
  ...
  }, [watchedTopicId, watchedLangId]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterTopic, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]);
  useEffect(() => { load(); setSelectedIds(new Set()); }, [searchDebounce, page, filterTopic, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]);

  async function refreshSummary() {
    const res = await api.getTableSummary('topic_translations');
  ...
  async function load() {
    ...
    const res = await api.listTopicTranslations('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  async function onSubmit(data: any) {
    ...
    fd.append('topic_id', String(data.topic_id));
    ...
    const res = editing
      ? await api.updateTopicTranslation(editing.id, fd, true)
      : await api.createTopicTranslation(fd, true);
    if (res.success) {
      toast.success(editing ? 'Topic translation updated' : 'Topic translation created');
      setDialogOpen(false); load(); refreshSummary();
    } else toast.error(res.error || 'Failed');
  }

  async function onSoftDelete(item: TopicTranslation) {
    if (!confirm(`Move "${item.name}" to trash?`)) return;
    setActionLoadingId(item.id);
    const res = await api.deleteTopicTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(item: TopicTranslation) {
    setActionLoadingId(item.id);
    const res = await api.restoreTopicTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`"${item.name}" restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(item: TopicTranslation) {
    if (!confirm(`PERMANENTLY delete "${item.name}"? This cannot be undone.`)) return;
    setActionLoadingId(item.id);
    const res = await api.permanentDeleteTopicTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(item: TopicTranslation) {
    const fd = new FormData();
    fd.append('is_active', String(!item.is_active));
    const res = await api.updateTopicTranslation(item.id, fd, true);
  ...
  async function handleBulkSoftDelete() {
    ...
    for (let i = 0; i < ids.length; i++) { const res = await api.deleteTopicTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkRestore() {
    ...
    setBulkActionLoading(true); const ids = Array.from(selectedIds); setBulkProgress({ done: 0, total: ids.length }); let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.restoreTopicTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkActionLoading(true); const ids = Array.from(selectedIds); setBulkProgress({ done: 0, total: ids.length }); let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.permanentDeleteTopicTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkGenerateContent() {
    ...
    try {
      ...
      items.filter(i => selectedIds.has(i.id)).forEach(i => {
        if (i.topic_id) parentIds.add(i.topic_id);
      });
  ...
  return (
    <div className="animate-fade-in">
      ...
      <PageHeader title="Topic Translations" description="Manage multi-language translations for topics"
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : BookOpen} title={showTrash ? 'Trash is empty' : 'No translations yet'} description={showTrash ? 'Deleted translations will appear here' : 'Add your first topic translation'}
            ...
            <TBody>
              {items.map(item => (
                <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">{item.topics?.slug ? <Badge variant="info">{(topics.find(t => t.id === item.topic_id) as any)?.english_name || item.topics.slug}</Badge> : <span className="text-slate-300">—</span>}</TD>
      ...
      {/* View Dialog */}
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} title="View Topic Translation" size="md">
        {viewItem && (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{(topics.find(t => t.id === viewItem.topic_id) as any)?.english_name || viewItem.topics?.slug || `Topic #${viewItem.topic_id}`}</Badge>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Topic Translation' : 'Add Topic Translation'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getTopicTranslation(editing.id);
          ...
          <div className="border border-indigo-200 rounded-lg overflow-hidden">
            ...
            {aiPanelOpen && (
              <div className="px-4 py-3 bg-white space-y-3">
                ...
                <AiProgressOverlay
                  active={aiLoading}
                  phases={['Analyzing topic content...', 'Generating translation with AI...', 'Processing response...', 'Preparing fields...']}
          ...
          <MultiLangField id="tt-name" label="Translated Name" placeholder="Topic name in target language" {...register('name', { required: true })} />
          <MultiLangField id="tt-short-intro" label="Short Intro" placeholder="Brief introduction..." multiline {...register('short_intro')} />
          <MultiLangField id="tt-long-intro" label="Long Intro" placeholder="Detailed introduction..." multiline {...register('long_intro')} />
          <MultiLangField id="tt-prerequisites" label="Prerequisites" placeholder="What learners should know before..." multiline {...register('prerequisites')} />
          <MultiLangField id="tt-learning-objectives" label="Learning Objectives" placeholder="What learners will achieve..." multiline {...register('learning_objectives')} />

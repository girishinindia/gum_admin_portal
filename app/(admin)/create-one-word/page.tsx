...
interface CoverageItem {
  ...
  has_synonym_translations: boolean;
}

interface OwListItem {
  id: number;
  code: string;
  slug: string;
...
export default function CreateOneWordPage() {
  ...
  const [isTrimWhitespace, setIsTrimWhitespace] = useState(true);
  ...
  // ── List state ──
  const [questions, setQuestions] = useState<OwListItem[]>([]);
  ...
  const [filterOwType, setFilterOwType] = useState('');
  ...
  const loadQuestions = useCallback(async () => {
    ...
    try {
      ...
      if (showTrash) {
        ...
        if (filterOwType) qs.set('question_type', filterOwType);
        if (filterDifficulty) qs.set('difficulty_level', filterDifficulty);
        if (filterStatus) qs.set('is_active', filterStatus);
      }
      if (searchDebounce) qs.set('search', searchDebounce);

      const r = await api.listOwQuestions('?' + qs.toString());
      if (r.success) {
        setQuestions(r.data || []);
  ...
  }, [topicId, page, pageSize, sortField, sortOrder, showTrash, searchDebounce, filterOwType, filterDifficulty, filterStatus]);
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [filterOwType, filterDifficulty, filterStatus, showTrash, searchDebounce, topicId, pageSize]);
  ...
  const loadLanguageIntoForm = (langId: number, translations: any[], synonymsData: any[]) => {
    const trans = translations.find((t: any) => t.language_id === langId);
    setQuestionText(trans?.question_text || '');
    setCorrectAnswer(trans?.correct_answer || '');
    setHintText(trans?.hint || '');
    setExplanationText(trans?.explanation || '');

    // Build synonyms with the selected language's text
    const syns: SynonymItem[] = synonymsData.map((syn: any) => {
      const synTrans = (syn.translations || []).find((t: any) => t.language_id === langId);
      return {
        id: syn.id,
  ...
  const startEdit = async (id: number) => {
    try {
      const r = await api.getOwQuestionFull(id);
      ...
      setIsCaseSensitive(q.is_case_sensitive ?? false);
      setIsTrimWhitespace(q.is_trim_whitespace ?? true);

      // Store all translations and synonyms for language switching
      const translations = q.question_translations || [];
      const synonymsData = q.synonyms || [];
      setAllTranslations(translations);
      setAllOptions(synonymsData);
      ...
      setCorrectAnswer(engTrans?.correct_answer || '');
      setHintText(engTrans?.hint || '');
      setExplanationText(engTrans?.explanation || '');

      // Build synonyms from enriched data (English)
      const syns: SynonymItem[] = synonymsData.map((syn: any) => {
        const engSynTrans = (syn.translations || []).find((t: any) => t.language_id === 7);
        return {
          id: syn.id,
          synonym_text: engSynTrans?.synonym_text || '',
        };
      });
      setSynonyms(syns);
  ...
  // ── Add synonym ──
  const addSynonym = () => {
    setSynonyms(prev => [...prev, { synonym_text: '' }]);
  };
  ...
  // ── Update synonym text ──
  const updateSynonymText = (idx: number, text: string) => {
    setSynonyms(prev => prev.map((s, i) => i === idx ? { ...s, synonym_text: text } : s));
  };
  ...
  const handleSaveTranslation = async () => {
    ...
    if (synonyms.some(s => !s.synonym_text.trim())) { toast.error('All synonyms must have text'); return; }
    ...
    try {
      ...
      if (existingQTrans && existingQTrans.id) {
        ...
        await api.updateOwQuestionTranslation(existingQTrans.id, {
        ...
        await api.createOwQuestionTranslation({
          one_word_question_id: editingId,
      ...
      // Save synonym translations
      for (let i = 0; i < synonyms.length; i++) {
        const syn = synonyms[i];
        const synonymId = allOptions[i]?.id;
        if (!synonymId) continue;

        const existingSynTrans = (allOptions[i]?.translations || []).find(
          (t: any) => t.language_id === editLangId
        );

        if (existingSynTrans && existingSynTrans.id) {
          // Update existing synonym translation
          await api.updateOwSynonymTranslation(existingSynTrans.id, {
            synonym_text: syn.synonym_text.trim(),
          });
        } else {
          // Create new synonym translation
          await api.createOwSynonymTranslation({
            one_word_synonym_id: synonymId,
            language_id: editLangId,
            synonym_text: syn.synonym_text.trim(),
          });
        }
      ...
      const r = await api.getOwQuestionFull(editingId);
      if (r.success && r.data) {
        const q = r.data;
        setAllTranslations(q.question_translations || []);
        setAllOptions(q.synonyms || []);
        setTranslationCoverage(q.translation_coverage || []);
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (mode === 'edit' && editingId) {
        r = await api.updateFullOwQuestion(editingId, payload);
      } else {
        r = await api.createFullOwQuestion(payload);
      }

      if (r.success) {
        toast.success(mode === 'edit' ? 'Question updated!' : 'Question created with synonyms & English translations!');
        resetForm();
        loadQuestions();
        loadSummary();
      } else {
  ...
  const handleAiTranslate = async (questionId?: number) => {
    ...
    try {
      const r = await api.autoTranslateOw({ question_ids: [qId] });
      if (r.success) {
        toast.success('AI translations generated!');
        // Reload coverage and translations
        if (editingId) {
          const full = await api.getOwQuestionFull(editingId);
          if (full.success) {
            setTranslationCoverage(full.data.translation_coverage || []);
            setAllTranslations(full.data.question_translations || []);
            setAllOptions(full.data.synonyms || []);
            // Reload current language into form
            loadLanguageIntoForm(editLangId, full.data.question_translations || [], full.data.synonyms || []);
          }
        }
        loadQuestions();
      } else {
  ...
  const handleAiTranslateAll = async () => {
    ...
    try {
      ...
      const r = await api.autoTranslateOw(payload);
      if (r.success) {
  ...
  const handleAiTranslateSelected = async () => {
    ...
    setAiTranslating(true);
    try {
      const r = await api.autoTranslateOw({ question_ids: Array.from(selectedIds) });
      if (r.success) {
  ...
  const handleSoftDelete = async (id: number) => {
    ...
    try {
      const r = await api.deleteOwQuestion(id);
      if (r.success) {
  ...
  const handleRestore = async (id: number) => {
    setActionLoadingId(id);
    try {
      const r = await api.restoreOwQuestion(id);
      if (r.success) {
  ...
  const handlePermanentDelete = async (id: number) => {
    ...
    setActionLoadingId(id);
    try {
      const r = await api.permanentDeleteOwQuestion(id);
      if (r.success) {
  ...
  const handleDeleteTranslation = async (langId: number) => {
    ...
    try {
      ...
      if (qTrans?.id) {
        await api.permanentDeleteOwQuestionTranslation(qTrans.id);
      }
      for (const syn of allOptions) {
        const synTrans = (syn.translations || []).find((t: any) => t.language_id === langId);
        if (synTrans?.id) {
          await api.permanentDeleteOwSynonymTranslation(synTrans.id);
        }
      }
      ...
      if (r.success && r.data) {
        ...
        setAllOptions(r.data.synonyms || []);
        setTranslationCoverage(r.data.translation_coverage || []);
      }
      if (editLangId === langId) {
        setEditLangId(7);
        loadLanguageIntoForm(7, r?.data?.question_translations || allTranslations, r?.data?.synonyms || allOptions);
      }
      loadSummary();
    } catch (e: any) {
  ...
  const handleDeleteAllTranslations = async () => {
    if (!editingId) return;
    const nonEngLangs = translationCoverage.filter(c => c.language_id !== 7 && (c.has_question_translation || c.has_synonym_translations));
    ...
    try {
      for (const lang of nonEngLangs) {
        ...
        for (const syn of allOptions) {
          const synTrans = (syn.translations || []).find((t: any) => t.language_id === lang.language_id);
          if (synTrans?.id) {
            await api.permanentDeleteOwSynonymTranslation(synTrans.id);
          }
        }
  ...
  const translatedLangs = translationCoverage.filter(c => c.has_question_translation && (synonyms.length === 0 || c.has_synonym_translations)).length;
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        ...
        description="Create, manage, and translate one-word questions in one place"
        actions={
          mode === 'list' && !showTrash ? (
      ...
      {mode === 'list' && (
        <DataToolbar
          ...
          searchPlaceholder={showTrash ? 'Search trash...' : 'Search OW questions...'}
        >
          {!showTrash && (
            <>
              ...
              <select className="h-10 px-3 pr-8 text-sm rounded-lg border border-slate-200 bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 focus:border-brand-500 appearance-none cursor-pointer" value={filterOwType} onChange={e => setFilterOwType(e.target.value)}>
                <option value="">All Types</option>
                {OW_TYPE_OPTIONS.map(t => <option key={t.value} value={t.value}>{t.label}</option>)}
              </select>
      ...
      {mode === 'list' ? (
        ...
        <div className="space-y-4">
          {listLoading ? (
            ...
            <EmptyState
              ...
              title={showTrash ? 'Trash is empty' : 'No OW questions yet'}
              description={showTrash ? 'No deleted questions' : (searchDebounce || filterOwType || filterDifficulty || filterStatus ? 'No questions match your filters' : topicId ? 'Add your first one-word question' : 'Select a subject, chapter, and topic to create questions — or browse all questions below')}
              action={!showTrash && !searchDebounce && !filterOwType && !filterDifficulty && !filterStatus && topicId ? <Button onClick={startCreate}><Plus className="w-4 h-4" /> Add question</Button> : undefined}
            />
          ) : (
                ...
                <TBody>
                  {questions.map(q => (
                    <TR key={q.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(q.id) && 'bg-brand-50/40')}>
                      ...
                      <TD className="py-2.5 max-w-[144px] truncate font-mono text-sm" title={q.correct_answer}>
                        {q.correct_answer || '—'}
                      </TD>
                      <TD className="py-2.5">
                        <Badge className={cn('text-xs', {
                          'bg-blue-50 text-blue-700': q.question_type === 'one_word',
                          'bg-violet-50 text-violet-700': q.question_type === 'fill_in_the_blank',
                          'bg-teal-50 text-teal-700': q.question_type === 'code_output',
                        })}>
                          {OW_TYPE_OPTIONS.find(t => t.value === q.question_type)?.label || q.question_type}
                        </Badge>
                      </TD>
                      <TD className="py-2.5">
                        <Badge className={cn('text-xs', {
                      ...
                      <TD className="py-2.5 text-center">{q.synonym_count}</TD>
                      <TD className="py-2.5">
                        {!showTrash ? (
        ...
        <div className="bg-white border rounded-lg">
            ...
            <div className="px-6 pt-3 border-b bg-gray-50">
              <div className="flex gap-1 overflow-x-auto">
                {availableLanguages.map(lang => {
                  ...
                  const hasTranslation = coverage?.has_question_translation && coverage?.has_synonym_translations;
                  return (
                    <button
                      key={lang.id}
          ...
          <div className="p-6 space-y-6">
            {/* Row 1: Type + Difficulty + Points/Order/Active/Required */}
            ...
            {/* Case Sensitive & Trim Whitespace checkboxes */}
            <div className="flex items-center gap-6">
              <label className={cn("flex items-center gap-1.5 text-sm", isNonEnglish ? "text-gray-400 cursor-not-allowed" : "cursor-pointer")}>
                <input type="checkbox" checked={isCaseSensitive} onChange={e => setIsCaseSensitive(e.target.checked)} className="rounded" disabled={isNonEnglish} />
                Case Sensitive
              </label>
              <label className={cn("flex items-center gap-1.5 text-sm", isNonEnglish ? "text-gray-400 cursor-not-allowed" : "cursor-pointer")}>
                <input type="checkbox" checked={isTrimWhitespace} onChange={e => setIsTrimWhitespace(e.target.checked)} className="rounded" disabled={isNonEnglish} />
                Trim Whitespace
              </label>
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Correct Answer ({currentLangName}) *
              </label>
              <Input
                value={correctAnswer}
                onChange={e => setCorrectAnswer(e.target.value)}
                placeholder={`Enter the correct answer in ${currentLangName}...`}
              />
            </div>
            ...
            <div>
              ...
              {synonyms.length === 0 ? (
                <p className="text-sm text-gray-400 italic">No synonyms added. {!isNonEnglish && 'Click "Add Synonym" to add alternative answers.'}</p>
              ) : (
                <div className="space-y-2">
                  {synonyms.map((syn, idx) => (
                    <div key={idx} className="flex items-center gap-2">
                      <span className="flex-shrink-0 w-8 h-8 rounded-full border-2 border-gray-300 flex items-center justify-center text-xs text-gray-400">
                        {idx + 1}
                      </span>
                      <Input
                        className="flex-1"
                        value={syn.synonym_text}
                        onChange={e => updateSynonymText(idx, e.target.value)}
                        placeholder={`Synonym ${idx + 1} in ${currentLangName}...`}
                      />
                      {!isNonEnglish && (
                        <button
              ...
              {isNonEnglish && (
                <p className="text-xs text-gray-400 mt-2">
                  Adding/removing synonyms can only be done in English.
                </p>
              )}
            </div>
              ...
              <div className="border rounded-lg p-4 bg-gray-50">
                ...
                <div className="flex flex-wrap gap-2">
                  {translationCoverage.map(c => {
                    const complete = c.has_question_translation && (synonyms.length === 0 || c.has_synonym_translations);
                    const partial = c.has_question_translation || c.has_synonym_translations;
                    return (

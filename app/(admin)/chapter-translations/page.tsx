            ...
            <TBody>
              {items.map(item => (
                <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">{item.chapters?.slug ? <Badge variant="info">{(chapters.find(c => c.id === item.chapter_id) as any)?.english_name || item.chapters.slug}</Badge> : <span className="text-slate-300">—</span>}</TD>
        ...
        {viewItem && (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{(chapters.find(c => c.id === viewItem.chapter_id) as any)?.english_name || viewItem.chapters?.slug || `Chapter #${viewItem.chapter_id}`}</Badge>

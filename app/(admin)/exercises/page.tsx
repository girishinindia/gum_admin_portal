...
interface Assessment {
  ...
  sub_topic?: { id: number; slug: string; english_name?: string } | null;
}

interface Subject { id: number; english_name?: string }
...
interface SubTopic { id: number; slug: string; english_name?: string; topic_id?: number }
...
export default function ExercisesPage() {
  ...
  const [dialogTopics, setDialogTopics] = useState<Topic[]>([]);
  ...
  // Toolbar cascade: subject → chapters
  useEffect(() => {
    setFilterChapter('');
    setFilterTopic('');
    setFilterSubTopic('');
  ...
  // Toolbar cascade: chapter → topics
  useEffect(() => {
    setFilterTopic('');
    setFilterSubTopic('');
    if (filterChapter) {
      api.listTopics(`?limit=500&is_active=true&chapter_id=${filterChapter}`).then(res => { if (res.success) setTopics(res.data || []); });
    } else {
      setTopics([]);
    }
  }, [filterChapter]);

  // Toolbar cascade: topic → sub-topics
  useEffect(() => {
    setFilterSubTopic('');
    if (filterTopic) {
      api.listSubTopics(`?limit=500&is_active=true&topic_id=${filterTopic}`).then(res => { if (res.success) setSubTopics(res.data || []); });
    } else {
      setSubTopics([]);
    }
  }, [filterTopic]);
  ...
  useEffect(() => { setPage(1); }, [searchDebounce, filterSubject, filterChapter, filterTopic, filterSubTopic, filterStatus, filterDifficulty, pageSize, showTrash]);

  // Load data
  useEffect(() => { load(); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterSubject, filterChapter, filterTopic, filterSubTopic, filterStatus, filterDifficulty, showTrash]);
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      else if (filterTopic) qs.set('topic_id', filterTopic);
  ...
  async function loadSummary() {
    const res: any = await api.listAssessments('?assessment_type=exercise&summary=true');
  ...
  function openEdit(item: Assessment) {
    setEditing(item);
    setDialogKey(k => k + 1);
    // Pre-populate dialog cascade from item's sub_topic relationship
    // We'll load the cascade from the sub_topic_id
    if (item.sub_topic_id) {
      // Load sub-topic details and cascade upward
      api.listSubTopics(`?limit=1&id=${item.sub_topic_id}`).then(async (res) => {
        if (res.success && res.data?.length > 0) {
          const st = res.data[0];
          const topicId = st.topic_id ? String(st.topic_id) : '';
          if (topicId) {
            const topicRes = await api.listTopics(`?limit=1&id=${topicId}`);
            if (topicRes.success && topicRes.data?.length > 0) {
              const topic = topicRes.data[0];
              const chapterId = topic.chapter_id ? String(topic.chapter_id) : '';
              if (chapterId) {
                const chapterRes = await api.listChapters(`?limit=1&id=${chapterId}`);
                if (chapterRes.success && chapterRes.data?.length > 0) {
                  const chapter = chapterRes.data[0];
                  const subjectId = chapter.subject_id ? String(chapter.subject_id) : '';
                  setDialogSubject(subjectId);
                  if (subjectId) {
                    const chaptersRes = await api.listChapters(`?limit=500&is_active=true&subject_id=${subjectId}`);
                    if (chaptersRes.success) setDialogChapters(chaptersRes.data || []);
                  }
                  setDialogChapter(chapterId);
                  const topicsRes = await api.listTopics(`?limit=500&is_active=true&chapter_id=${chapterId}`);
                  if (topicsRes.success) setDialogTopics(topicsRes.data || []);
                  setDialogTopic(topicId);
                  const subTopicsRes = await api.listSubTopics(`?limit=500&is_active=true&topic_id=${topicId}`);
                  if (subTopicsRes.success) setDialogSubTopics(subTopicsRes.data || []);
                }
              }
            }
          }
    ...
    reset({
      sub_topic_id: item.sub_topic_id ? String(item.sub_topic_id) : '',
  ...
  async function onSubmit(data: any) {
    const payload: Record<string, any> = {
      ...
      sub_topic_id: data.sub_topic_id ? Number(data.sub_topic_id) : null,
  ...
  async function onSoftDelete(item: Assessment) {
    ...
    if (res.success) { toast.success('Exercise moved to trash'); load(); loadSummary(); }
  ...
  async function onRestore(item: Assessment) {
    ...
    setActionLoadingId(null);
    if (res.success) { toast.success('Exercise restored'); load(); loadSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  async function onPermanentDelete(item: Assessment) {
    ...
    setActionLoadingId(null);
    if (res.success) { toast.success('Exercise permanently deleted'); load(); loadSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Exercises"
        description="Manage exercise assessments"
        actions={
            ...
            {!showTrash && (
              <>
                <button
                  ...
                  title={filterSubTopic ? 'AI Generate Exercises' : 'Select a sub-topic first'}
                  className={cn(
      ...
      {/* Toolbar with cascade filters */}
      <DataToolbar ref={toolbarRef} search={search} onSearchChange={setSearch} searchPlaceholder={showTrash ? 'Search trash...' : 'Search exercises...'}>
        {!showTrash && (
          <>
            <SearchableSelect
            ...
            />
            <SearchableSelect
              options={chapters.map(c => ({ value: String(c.id), label: c.english_name || '' }))}
              value={filterChapter}
            ...
            />
            <SearchableSelect
              options={topics.map(t => ({ value: String(t.id), label: t.english_name || '' }))}
              value={filterTopic}
            ...
            />
            <SearchableSelect
              options={subTopics.map(st => ({ value: String(st.id), label: st.english_name || st.slug || '' }))}
              value={filterSubTopic}
              onChange={setFilterSubTopic}
              placeholder={filterTopic ? 'All sub-topics' : 'Select topic first'}
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : FileQuestion} title={showTrash ? 'Trash is empty' : 'No exercises yet'}
          description={showTrash ? 'No deleted exercises' : (searchDebounce || filterSubTopic || filterStatus || filterDifficulty ? 'No exercises match your filters' : 'Add your first exercise')}
          action={!showTrash && !searchDebounce && !filterSubTopic && !filterStatus && !filterDifficulty ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add Exercise</Button> : undefined} />
      ) : (
                ...
                <TR key={item.id} className={showTrash ? 'bg-amber-50/30' : undefined}>
                  ...
                  <TD className="py-2.5">
                    <span className={cn('text-sm font-medium', showTrash ? 'text-slate-500 line-through' : 'text-slate-900')}>
                      {item.english_title || item.title || `Exercise #${item.id}`}
                    </span>
                  </TD>
                  <TD className="py-2.5">
                    <span className="text-sm text-slate-600">
                      {item.sub_topic?.english_name || item.sub_topic?.slug || (item.sub_topic_id ? `ID: ${item.sub_topic_id}` : '—')}
                    </span>
                  </TD>
                  <TD className="py-2.5">
      ...
      {/* View Dialog */}
      <Dialog open={!!viewing} onClose={() => setViewing(null)} title="Exercise Details" size="md">
        {viewing && (
          <div className="p-6">
            <div className="flex items-center gap-4 mb-6">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">{viewing.english_title || viewing.title || `Exercise #${viewing.id}`}</h3>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              <DetailRow label="Sub-Topic" value={viewing.sub_topic?.english_name || viewing.sub_topic?.slug || (viewing.sub_topic_id ? `ID: ${viewing.sub_topic_id}` : undefined)} />
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-slate-900">AI Generate Exercises</h3>
              </div>
            ...
            <form onSubmit={handleAiGenerate} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Number of Exercises <span className="text-slate-400 font-normal">(0 = AI decides)</span></label>
              ...
              {aiResult && (
                <div className={cn('px-3 py-2 rounded-lg text-sm', aiResult.success ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200')}>
                  {aiResult.success ? `Successfully generated ${aiResult.count} exercise(s)!` : aiResult.error}
                </div>
              )}
      ...
      {/* Create / Edit Dialog */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Exercise' : 'Add Exercise'} size="md">
        <form key={dialogKey} onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          {/* Cascade: Subject → Chapter → Topic → Sub-Topic */}
          <SearchableSelect
            label="Subject"
            options={subjects.map(s => ({ value: String(s.id), label: s.english_name || '' }))}
            value={dialogSubject}
          ...
          <SearchableSelect
            ...
            onChange={(val) => {
              ...
              setDialogSubTopics([]);
              if (val) {
                api.listTopics(`?limit=500&is_active=true&chapter_id=${val}`).then(res => {
                  if (res.success) setDialogTopics(res.data || []);
                  else setDialogTopics([]);
                });
              } else {
                setDialogTopics([]);
              }
          ...
          <SearchableSelect
            label="Topic"
            options={dialogTopics.map(t => ({ value: String(t.id), label: t.english_name || '' }))}
            value={dialogTopic}
            onChange={(val) => {
              setDialogTopic(val);
              setValue('sub_topic_id', '');
              if (val) {
                api.listSubTopics(`?limit=500&is_active=true&topic_id=${val}`).then(res => {
                  if (res.success) setDialogSubTopics(res.data || []);
                  else setDialogSubTopics([]);
                });
              } else {
                setDialogSubTopics([]);
              }
          ...
          <SearchableSelect
            label="Sub-Topic *"
            options={dialogSubTopics.map(st => ({ value: String(st.id), label: st.english_name || st.slug || '' }))}
            value={watch('sub_topic_id') || ''}
            onChange={(val) => setValue('sub_topic_id', val)}
            placeholder={dialogTopic ? 'Select a sub-topic' : 'Select topic first'}
            searchPlaceholder="Search sub-topics..."

...
interface Assessment {
  ...
  topic?: { id: number; english_name?: string } | null;
}
...
export default function AssignmentsPage() {
  ...
  useEffect(() => { setPage(1); }, [searchDebounce, filterSubject, filterChapter, filterTopic, filterStatus, filterDifficulty, pageSize, showTrash]);

  // Load data
  useEffect(() => { load(); }, [searchDebounce, page, pageSize, sortField, sortOrder, filterSubject, filterChapter, filterTopic, filterStatus, filterDifficulty, showTrash]);
  ...
  async function loadSummary() {
    const res: any = await api.listAssessments('?assessment_type=assignment&summary=true');
  ...
  async function handleAiGenerate(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (res.success) {
        ...
        toast.success(`AI generated ${count} assignment(s)`);
        load();
        loadSummary();
      } else {
  ...
  async function handleAiTranslateAll() {
    if (!filterTopic || items.length === 0) { toast.error('Select a topic with assignments first'); return; }
  ...
  function openEdit(item: Assessment) {
    setEditing(item);
    setDialogKey(k => k + 1);
    // Pre-populate dialog cascade from item's topic relationship
    if (item.topic_id) {
      // Load topic details and cascade upward
      api.listTopics(`?limit=1&id=${item.topic_id}`).then(async (res) => {
  ...
  async function onSoftDelete(item: Assessment) {
    ...
    if (res.success) { toast.success('Assignment moved to trash'); load(); loadSummary(); }
  ...
  async function onRestore(item: Assessment) {
    ...
    setActionLoadingId(null);
    if (res.success) { toast.success('Assignment restored'); load(); loadSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  async function onPermanentDelete(item: Assessment) {
    ...
    setActionLoadingId(null);
    if (res.success) { toast.success('Assignment permanently deleted'); load(); loadSummary(); }
    else toast.error(res.error || 'Failed');
  }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Assignments"
        description="Manage assignment assessments"
        actions={
            ...
            {!showTrash && (
              <>
                <button
                  ...
                  title={filterTopic ? 'AI Generate Assignments' : 'Select a topic first'}
                  className={cn(
                ...
                <button
                  ...
                  disabled={!filterTopic || aiTranslating || items.length === 0}
                  title={filterTopic ? 'AI Translate all assignments in this topic' : 'Select a topic first'}
                  className={cn(
                    'inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg transition-colors',
                    filterTopic && !aiTranslating && items.length > 0
      ...
      <DataToolbar ref={toolbarRef} search={search} onSearchChange={setSearch} searchPlaceholder={showTrash ? 'Search trash...' : 'Search assignments...'}>
        {!showTrash && (
          <>
            <SearchableSelect
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : FileQuestion} title={showTrash ? 'Trash is empty' : 'No assignments yet'}
          description={showTrash ? 'No deleted assignments' : (searchDebounce || filterTopic || filterStatus || filterDifficulty ? 'No assignments match your filters' : 'Add your first assignment')}
          action={!showTrash && !searchDebounce && !filterTopic && !filterStatus && !filterDifficulty ? <Button onClick={openCreate}><Plus className="w-4 h-4" /> Add Assignment</Button> : undefined} />
      ) : (
                ...
                <TR key={item.id} className={showTrash ? 'bg-amber-50/30' : undefined}>
                  ...
                  <TD className="py-2.5">
                    <span className="text-sm text-slate-600">
                      {item.topic?.english_name || (item.topic_id ? `ID: ${item.topic_id}` : '—')}
                    </span>
                  </TD>
                  <TD className="py-2.5">
        ...
        {viewing && (
          <div className="p-6">
            <div className="flex items-center gap-4 mb-6">
              <div>
                <h3 className="text-lg font-semibold text-slate-900">{viewing.english_title || viewing.title || `Assignment #${viewing.id}`}</h3>
            ...
            <div className="grid grid-cols-2 gap-x-8 gap-y-4">
              <DetailRow label="Topic" value={viewing.topic?.english_name || (viewing.topic_id ? `ID: ${viewing.topic_id}` : undefined)} />
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-md mx-4">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-lg font-semibold text-slate-900">AI Generate Assignments</h3>
              </div>
            ...
            <form onSubmit={handleAiGenerate} className="p-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Number of Assignments <span className="text-slate-400 font-normal">(0 = AI decides)</span></label>
              ...
              {aiResult && (
                <div className={cn('px-3 py-2 rounded-lg text-sm', aiResult.success ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : 'bg-red-50 text-red-700 border border-red-200')}>
                  {aiResult.success ? `Successfully generated ${aiResult.count} assignment(s)!` : aiResult.error}
                </div>
              )}

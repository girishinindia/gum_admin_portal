...
interface OwSynonymTranslation {
  id: number;
  ...
  created_at: string;
  updated_at: string;
  one_word_synonyms?: { one_word_question_id: number; display_order: number } | null;
...
type SortField = 'id' | 'created_at' | 'one_word_synonym_id' | 'language_id';
...
export default function OwSynonymTranslationsPage() {
  const searchParams = useSearchParams();
  const [items, setItems] = useState<OwSynonymTranslation[]>([]);
  const [owSynonyms, setOwSynonyms] = useState<OwSynonym[]>([]);
  const [languages, setLanguages] = useState<Language[]>([]);
  const [coverage, setCoverage] = useState<CoverageItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<OwSynonymTranslation | null>(null);
  const [dialogKey, setDialogKey] = useState(0);
  const [subjects, setSubjects] = useState<any[]>([]);
  const [chapters, setChapters] = useState<any[]>([]);
  const [topics, setTopics] = useState<any[]>([]);
  const [owQuestions, setOwQuestions] = useState<any[]>([]);
  const [filterSubjectId, setFilterSubjectId] = useState('');
  const [filterChapterId, setFilterChapterId] = useState('');
  const [filterTopicId, setFilterTopicId] = useState('');
  const [filterQuestionId, setFilterQuestionId] = useState('');
  const [filterSynonymId, setFilterSynonymId] = useState(searchParams.get('one_word_synonym_id') || '');
  ...
  const [viewItem, setViewItem] = useState<OwSynonymTranslation | null>(null);

  const toolbarRef = useRef<DataToolbarHandle>(null);
  const router = useRouter();

  // Auto-fetch translation when synonym or language changes
  const watchedSynonymId = watch('one_word_synonym_id');
  ...
  useEffect(() => {
    ...
    if (!watchedSynonymId || !watchedLangId) return;

    let cancelled = false;
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?one_word_synonym_id=${watchedSynonymId}&language_id=${watchedLangId}&limit=1`;
      const res = await api.listOwSynonymTranslations(qs);
      if (cancelled) return;

      if (res.success && res.data && res.data.length > 0) {
        const item = res.data[0] as OwSynonymTranslation;
        setEditing(item);
  ...
  }, [watchedSynonymId, watchedLangId]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    api.listOwSynonyms('?limit=999&sort=display_order&order=asc').then(res => { if (res.success) setOwSynonyms(res.data || []); });
  ...
  // Cascade: when topic changes, load OW questions
  useEffect(() => {
    setFilterQuestionId('');
    setFilterSynonymId('');
    if (filterTopicId) {
      api.listOwQuestions(`?limit=999&sort=display_order&order=asc&topic_id=${filterTopicId}`).then(res => {
  ...
  // Cascade: when question changes, reload synonyms filtered by question
  useEffect(() => {
    setFilterSynonymId('');
    if (filterQuestionId) {
      api.listOwSynonyms(`?one_word_question_id=${filterQuestionId}&limit=999&sort=display_order&order=asc`).then(res => {
        if (res.success) setOwSynonyms(res.data || []);
        else setOwSynonyms([]);
      });
    } else {
      api.listOwSynonyms('?limit=999&sort=display_order&order=asc').then(res => { if (res.success) setOwSynonyms(res.data || []); });
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterSubjectId, filterChapterId, filterTopicId, filterQuestionId, filterSynonymId, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]);
  useEffect(() => { load(); setSelectedIds(new Set()); }, [searchDebounce, page, filterSubjectId, filterChapterId, filterTopicId, filterQuestionId, filterSynonymId, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]); // eslint-disable-line react-hooks/exhaustive-deps

  async function refreshSummary() {
    const res = await api.getTableSummary('one_word_synonym_translations');
    if (res.success && Array.isArray(res.data) && res.data.length > 0) setSummary(res.data[0]);
  }

  async function loadCoverage() {
    const res = await api.getOwSynonymTranslationCoverage();
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterSynonymId) qs.set('one_word_synonym_id', filterSynonymId);
      if (filterLanguage) qs.set('language_id', filterLanguage);
      if (filterStatus) qs.set('is_active', filterStatus);
    }
    qs.set('sort', sortField);
    qs.set('order', sortOrder);
    const res = await api.listOwSynonymTranslations('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  function clearFormFields() {
    const sid = getValues('one_word_synonym_id');
    const lid = getValues('language_id');
    Object.entries(defaultFormValues).forEach(([k, v]) => setValue(k, v));
    setValue('one_word_synonym_id', sid);
    setValue('language_id', lid);
  }

  function populateForm(item: OwSynonymTranslation) {
    setValue('synonym_text', item.synonym_text || '');
  ...
  function openCreate() {
    ...
    reset({ ...defaultFormValues, one_word_synonym_id: owSynonyms[0]?.id || '', language_id: languages[0]?.id || '' });
    setDialogOpen(true);
  }

  function openEdit(item: OwSynonymTranslation) {
    skipAutoFetchRef.current = true;
    setEditing(item); setFormMode('existing'); setDialogKey(k => k + 1);
    reset({
      one_word_synonym_id: item.one_word_synonym_id,
      language_id: item.language_id,
      synonym_text: item.synonym_text || '',
  ...
  async function onSubmit(data: any) {
    ...
    // Scalar fields
    const scalarFields = ['one_word_synonym_id', 'language_id', 'synonym_text'];
    scalarFields.forEach(k => {
      if (data[k] !== undefined && data[k] !== null && data[k] !== '') {
        payload[k] = ['one_word_synonym_id', 'language_id'].includes(k) ? Number(data[k]) : data[k];
      }
    });
    ...
    const res = editing
      ? await api.updateOwSynonymTranslation(editing.id, payload)
      : await api.createOwSynonymTranslation(payload);
    if (res.success) {
  ...
  async function onSoftDelete(item: OwSynonymTranslation) {
    if (!confirm(`Move translation #${item.id} to trash? You can restore it later.`)) return;
    setActionLoadingId(item.id);
    const res = await api.deleteOwSynonymTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Translation moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(item: OwSynonymTranslation) {
    setActionLoadingId(item.id);
    const res = await api.restoreOwSynonymTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`Translation #${item.id} restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(item: OwSynonymTranslation) {
    if (!confirm(`PERMANENTLY delete translation #${item.id}? This cannot be undone.`)) return;
    setActionLoadingId(item.id);
    const res = await api.permanentDeleteOwSynonymTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Translation permanently deleted'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onToggleActive(item: OwSynonymTranslation) {
    const res = await api.updateOwSynonymTranslation(item.id, { is_active: !item.is_active });
    if (res.success) { toast.success(`${!item.is_active ? 'Activated' : 'Deactivated'}`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  function openView(item: OwSynonymTranslation) {
    setViewItem(item);
    setViewOpen(true);
  }
  ...
  async function handleBulkSoftDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.deleteOwSynonymTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkRestore() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.restoreOwSynonymTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.permanentDeleteOwSynonymTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  function getSynonymLabel(synonymId: number): string {
    const syn = owSynonyms.find(s => s.id === synonymId);
    if (!syn) return `Synonym #${synonymId}`;
    return `Synonym #${syn.id} (Q${syn.one_word_question_id}, Order: ${syn.display_order})`;
  }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader title="One Word Synonym Translations" description="Manage translations for one word synonyms"
      ...
      <DataToolbar ref={toolbarRef} search={search} onSearchChange={setSearch} searchPlaceholder={showTrash ? 'Search trash...' : 'Search translations...'}>
        {!showTrash && (
          <>
            ...
            <select className={selectClass} value={filterSynonymId} onChange={e => setFilterSynonymId(e.target.value)}>
              <option value="">All Synonyms</option>
              {owSynonyms.map(s => <option key={s.id} value={s.id}>{s.display_order ? s.display_order + '. ' : ''}Synonym {s.id}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState icon={showTrash ? Trash2 : Languages} title={showTrash ? 'Trash is empty' : 'No translations yet'} description={showTrash ? 'Deleted translations will appear here' : 'Add your first synonym translation'}
            ...
            <Table>
              <THead>
                <TR className="hover:bg-transparent">
                  ...
                  <TH><button onClick={() => handleSort('one_word_synonym_id')} className="inline-flex items-center gap-1.5 hover:text-slate-900 transition-colors cursor-pointer">Synonym <SortIcon field="one_word_synonym_id" /></button></TH>
              ...
              <TBody>
                {items.map(item => (
                  <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                    ...
                    <TD className="py-2.5">
                      {item.one_word_synonyms ? (
                        <Badge variant="info">Syn #{item.one_word_synonym_id} (Q{item.one_word_synonyms.one_word_question_id})</Badge>
                      ) : (
                        <Badge variant="muted">Synonym #{item.one_word_synonym_id}</Badge>
                      )}
                    </TD>
                    ...
                    <TD className="py-2.5"><span className={cn('text-sm', showTrash ? 'text-slate-500 line-through' : 'text-slate-900')}>{item.synonym_text ? (item.synonym_text.length > 60 ? item.synonym_text.substring(0, 60) + '...' : item.synonym_text) : <span className="text-slate-300 italic">Empty</span>}</span></TD>
      ...
      {/* --- View Dialog --- */}
      <Dialog open={viewOpen} onClose={() => setViewOpen(false)} title="View Synonym Translation" size="lg">
        {viewItem && (
          <div className="p-6 space-y-4">
            {/* Header info */}
            <div className="flex items-center gap-3 flex-wrap">
              <Badge variant="info">{getSynonymLabel(viewItem.one_word_synonym_id)}</Badge>
            ...
            {/* Synonym details */}
            {viewItem.one_word_synonyms && (
              <div className="bg-slate-50 rounded-lg px-4 py-3 space-y-1">
                <span className="block text-xs font-medium text-slate-500">Synonym Details</span>
                <div className="flex items-center gap-3 text-sm text-slate-700">
                  <span>Question ID: {viewItem.one_word_synonyms.one_word_question_id}</span>
                  <span>Order: {viewItem.one_word_synonyms.display_order}</span>
                </div>
              </div>
            )}

            <ViewField label="Synonym Text" value={viewItem.synonym_text} />
      ...
      {/* --- Edit / Create Dialog --- */}
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Synonym Translation' : 'Add Synonym Translation'} size="lg">
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-lg px-4 py-3 -mt-1">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  const refreshed = await api.getOwSynonymTranslation(editing.id);
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">Synonym</label>
              <select className={cn(selectClass, 'w-full')} {...register('one_word_synonym_id', { required: true })}>
                {owSynonyms.map(s => <option key={s.id} value={s.id}>{s.display_order ? s.display_order + '. ' : ''}Synonym {s.id} (Q{s.one_word_question_id})</option>)}
              </select>
            </div>
            <div>
          ...
          {/* Synonym text */}
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">Synonym Text</label>
            <textarea className={cn(selectClass, 'w-full min-h-[100px]')} placeholder="Translated synonym text..." {...register('synonym_text', { required: true })} />
          </div>

...
interface WebinarTranslation {
  ...
  created_at: string;
  updated_at: string;
  webinars?: { title?: string; code?: string; slug?: string; course_id?: number } | null;
...
export default function WebinarTranslationsPage() {
  const [items, setItems] = useState<WebinarTranslation[]>([]);
  const [webinars, setWebinars] = useState<Webinar[]>([]);
  const [languages, setLanguages] = useState<Language[]>([]);
  const [loading, setLoading] = useState(true);
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editing, setEditing] = useState<WebinarTranslation | null>(null);
  ...
  const [viewItem, setViewItem] = useState<WebinarTranslation | null>(null);
  const [viewTab, setViewTab] = useState<typeof TABS[number]>('Basic');

  const toolbarRef = useRef<DataToolbarHandle>(null);
  const router = useRouter();

  // ─── Auto-fetch translation when webinar or language changes ───
  const watchedWebinarId = watch('webinar_id');
  ...
  useEffect(() => {
    ...
    if (!watchedWebinarId || !watchedLangId) return;

    let cancelled = false;
    const fetchTranslation = async () => {
      setFormLoading(true);
      const qs = `?webinar_id=${watchedWebinarId}&language_id=${watchedLangId}&limit=1`;
      const res = await api.listWebinarTranslations(qs);
      if (cancelled) return;

      if (res.success && res.data && res.data.length > 0) {
        const item = res.data[0] as WebinarTranslation;
        setEditing(item);
  ...
  }, [watchedWebinarId, watchedLangId]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    api.listWebinars('?limit=500&sort=title&order=asc').then(res => { if (res.success) setWebinars((res.data || []).filter((b: Webinar) => !b.deleted_at)); });
  ...
  useEffect(() => { setPage(1); setSelectedIds(new Set()); }, [searchDebounce, filterWebinar, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]);
  useEffect(() => { load(); setSelectedIds(new Set()); }, [searchDebounce, page, filterWebinar, filterLanguage, filterStatus, sortField, sortOrder, pageSize, showTrash]); // eslint-disable-line react-hooks/exhaustive-deps

  async function refreshSummary() {
    const res = await api.getTableSummary('webinar_translations');
  ...
  async function load() {
    ...
    if (showTrash) {
      ...
      if (filterWebinar) qs.set('webinar_id', filterWebinar);
      if (filterLanguage) qs.set('language_id', filterLanguage);
      if (filterStatus) qs.set('is_active', filterStatus);
    }
    qs.set('sort', sortField);
    qs.set('order', sortOrder);
    const res = await api.listWebinarTranslations('?' + qs.toString());
    if (res.success) {
      setItems(res.data || []);
  ...
  function openCreate() {
    ...
    reset({ ...defaultFormValues, webinar_id: webinars[0]?.id || '', language_id: languages[0]?.id || '' });
    setDialogOpen(true);
  }

  function openEdit(item: WebinarTranslation) {
    skipAutoFetchRef.current = true;
    setEditing(item); setFormMode('existing'); resetImageState(); setDialogKey(k => k + 1); setActiveTab('Basic');
    reset({
      webinar_id: item.webinar_id, language_id: item.language_id,
  ...
  async function onSubmit(data: any) {
    ...
    // Scalar text fields
    const scalarFields = [
      'webinar_id', 'language_id', 'title', 'short_description', 'description',
    ...
    const res = editing
      ...
      : await api.createWebinarTranslation(fd, true);
    if (res.success) {
  ...
  async function onSoftDelete(item: WebinarTranslation) {
    if (!confirm(`Move "${item.title}" to trash?`)) return;
    setActionLoadingId(item.id);
    const res = await api.softDeleteWebinarTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success('Translation moved to trash'); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onRestore(item: WebinarTranslation) {
    setActionLoadingId(item.id);
    const res = await api.restoreWebinarTranslation(item.id);
    setActionLoadingId(null);
    if (res.success) { toast.success(`"${item.title}" restored`); load(); refreshSummary(); }
    else toast.error(res.error || 'Failed');
  }

  async function onPermanentDelete(item: WebinarTranslation) {
  ...
  function openView(item: WebinarTranslation) {
    setViewItem(item);
    setViewTab('Basic');
    setViewOpen(true);
  }
  ...
  async function handleBulkSoftDelete() {
    ...
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.softDeleteWebinarTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkRestore() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.restoreWebinarTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkProgress({ done: 0, total: ids.length });
    let ok = 0;
    for (let i = 0; i < ids.length; i++) { const res = await api.deleteWebinarTranslation(ids[i]); if (res.success) ok++; setBulkProgress({ done: i + 1, total: ids.length }); }
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        title="Webinar Translations"
        description="Manage multilingual translations for course webinars"
        actions={
      ...
      <DataToolbar ref={toolbarRef} search={search} onSearchChange={setSearch} searchPlaceholder={showTrash ? 'Search trash...' : 'Search translations...'}>
        {!showTrash && (
          <>
            <select className={selectClass} value={filterWebinar} onChange={e => setFilterWebinar(e.target.value)}>
              <option value="">All Webinars</option>
              {webinars.map(b => <option key={b.id} value={b.id}>{b.title || b.code || `Webinar #${b.id}`}</option>)}
            </select>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          description={showTrash ? 'No deleted translations' : (searchDebounce || filterWebinar || filterLanguage || filterStatus ? 'No translations match your filters' : 'Add your first webinar translation')}
            ...
            <TBody>
              {items.map(item => (
                <TR key={item.id} className={cn(showTrash ? 'bg-amber-50/30' : undefined, selectedIds.has(item.id) && 'bg-brand-50/40')}>
                  ...
                  <TD className="py-2.5">
                    <span className="text-sm text-slate-600">{item.webinars?.title || item.webinars?.code || `#${item.webinar_id}`}</span>
                  </TD>
                  <TD className="py-2.5">
        ...
        <Dialog open={viewOpen} onClose={() => setViewOpen(false)} title={viewItem.title || 'Translation'} size="lg">
          ...
          {viewTab === 'Basic' && (
            <div className="space-y-3">
              ...
              <ViewField label="Webinar" value={viewItem.webinars?.title || viewItem.webinars?.code || `#${viewItem.webinar_id}`} />
      ...
      {/* Create/Edit Dialog */}
      <Dialog key={dialogKey} open={dialogOpen} onClose={() => setDialogOpen(false)} title={editing ? 'Edit Webinar Translation' : 'Create Webinar Translation'} size="lg">
          ...
          {activeTab === 'Basic' && (
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Webinar *</label>
                  <select className={cn(selectClass, 'w-full')} {...register('webinar_id', { required: true })}>

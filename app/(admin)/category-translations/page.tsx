...
export default function CategoryTranslationsPage() {
  ...
  async function handleBulkSoftDelete() {
    ...
    let ok = 0;
    for (const id of selectedIds) { const res = await api.deleteCategoryTranslation(id); if (res.success) ok++; }
  ...
  async function handleBulkRestore() {
    ...
    setBulkActionLoading(true);
    let ok = 0;
    for (const id of selectedIds) { const res = await api.restoreCategoryTranslation(id); if (res.success) ok++; }
  ...
  async function handleBulkPermanentDelete() {
    ...
    setBulkActionLoading(true);
    let ok = 0;
    for (const id of selectedIds) { const res = await api.permanentDeleteCategoryTranslation(id); if (res.success) ok++; }
